...
export function Header({ userEmail }: HeaderProps) {
  ...
  return <><header className="sticky top-0 z-20 flex h-16 items-center gap-2 border-b bg-background/90 px-3 backdrop-blur sm:gap-3 sm:px-4 md:px-5 lg:px-6">
    ...
    <div className="flex h-9 w-32 items-center rounded-md bg-[#080808] px-2.5 lg:hidden"><BrandLogo className="w-full" surface="dark" /></div>
    ...
    <Button aria-label="Cambiar tema" onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")} size="icon" variant="ghost">{mounted && resolvedTheme === "dark" ? <Sun className="size-4" /> : <Moon className="size-4" />}</Button>

...
export default async function SignAgreementPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const project = agreement.projects as unknown as { name:string;event_date:string;customers:{first_name:string;last_name:string};project_services:Array<{service_name:string}> };
  ...
  return <CustomerSigningExperience agreementVersion={agreement.template_version} clauses={clauses} customer={`${project.customers.first_name} ${project.customers.last_name}`} eventDate={new Intl.DateTimeFormat("es-CL",{dateStyle:"long",timeZone:"UTC"}).format(new Date(`${project.event_date}T12:00:00Z`))} project={project.name} service={project.project_services.map((item)=>item.service_name).join(" · ")||"Experiencia BOOMBOX"} token={token}/>;
}

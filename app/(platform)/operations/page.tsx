...
type PlanningRole={code:"OPERATOR"|"ASSEMBLY"|"DISASSEMBLY";label:string;pay:number;paymentId:string|null;staffId:string|null;assignee:string|null;status:string};
type PlanningEvent={id:string;date:string;customer:string;service:string;hours:number;address:string;district:string;venue:string;booths:number;roles:{operator:PlanningRole;assembly:PlanningRole;disassembly:PlanningRole};status:string;published:boolean};
type PlanningRequest={id:string;staffName:string;customer:string;service:string;date:string;responsibility:string};
...
export default async function OperationsPage() {
  ...
  return <div className="space-y-6"><StaffOperationsView events={planningEvents.map(event=>({id:event.id,date:event.date,customer:event.customer,service:`${event.service} · ${event.hours} horas`,published:event.published,assignments:[event.roles.operator,event.roles.assembly,event.roles.disassembly].filter(role=>role.assignee).map(role=>`${role.label}: ${role.assignee}`),settlements:(payrollResult.data??[]).filter(item=>item.project_id===event.id&&item.status!=="CANCELLED").map(item=>{const member=Array.isArray(item.staff)?item.staff[0]:item.staff;return{id:item.id,staff:member?`${member.first_name} ${member.last_name}`:"Staff",net:Number(item.total_internal_payment),paid:Number(item.paid_amount),status:item.settlement_status}}),eventStatus:event.status,requestCount:pendingRequests.filter(request=>(staffRequestsResult.data??[]).find(item=>item.id===request.id)?.project_id===event.id).length}))} requests={pendingRequests.map(request=>({id:request.id,projectId:(staffRequestsResult.data??[]).find(item=>item.id===request.id)?.project_id??"",staff:request.staffName,responsibility:request.responsibility,event:request.customer,date:request.date}))}/><FounderWorkspaceExperience currentDate={currentDate} founderName="Matías" initialPreferences={founderWorkspace} pendingTasks={taskSummary.pending} todayEvents={projects.filter(project=>project.event.date===today).length} values={workspaceValues}/></div>;
}

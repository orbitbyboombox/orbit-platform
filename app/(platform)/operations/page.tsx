...
export default async function OperationsPage() {
  ...
  const costMap=new Map((costMasterResult.data??[]).map(item=>[item.code,Number(item.amount)]));
  const planningEvents:PlanningEvent[]=projects.filter(project=>Boolean(project.event.date&&project.event.date>=today&&project.event.date<=planningEnd)).map(project=>{const projectAssignments=(assignmentsResult.data??[]).filter(item=>item.project_id===project.id&&!['CANCELLED','REJECTED'].includes(item.status));const hours=Number(project.event.durationHours??4);const findRole=(code:"OPERATOR"|"ASSEMBLY"|"DISASSEMBLY",label:string)=>{const assignment=projectAssignments.find(item=>item.assignment_type===code);const person=assignment?.staff?(Array.isArray(assignment.staff)?assignment.staff[0]:assignment.staff):null;const payment=(payrollResult.data??[]).find(item=>item.project_id===project.id&&item.staff_id===assignment?.staff_id);const stored=code==='OPERATOR'?payment?.operator_payment:code==='ASSEMBLY'?payment?.assembly_payment:payment?.disassembly_payment;const fallback=costMap.get(code==='OPERATOR'?`OPERATOR_${hours}_HOURS`:code)??0;return{code,label,pay:Number(stored??fallback),paymentId:payment?.id??null,staffId:assignment?.staff_id??null,assignee:person?`${person.first_name} ${person.last_name}`:null,status:assignment?.status??'UNASSIGNED'}};const finance=financeByProject.get(project.id)??{};return{id:project.id,date:project.event.date,customer:project.client.name,service:project.services.join(' + ')||'Servicio BOOMBOX',hours,address:project.event.location,district:project.event.city,venue:String(finance.venue??project.event.location),booths:Number(finance.booths??1),roles:{operator:findRole('OPERATOR','Operador'),assembly:findRole('ASSEMBLY','Montaje'),disassembly:findRole('DISASSEMBLY','Desmontaje')},status:project.status}});
  const staffOptions=staff.map(member=>({id:member.id,name:`${member.first_name} ${member.last_name}`,skills:member.capabilities??[]}));
  const configuredPay={operator:costMap.get('OPERATOR_4_HOURS')??40000,assembly:costMap.get('ASSEMBLY')??7000,disassembly:costMap.get('DISASSEMBLY')??7000,combined:costMap.get('ASSEMBLY_DISASSEMBLY')??15000};
  return <div className="space-y-6"><OperationsPlanningBoard configuredPay={configuredPay} events={planningEvents} staff={staffOptions}/><FounderWorkspaceExperience currentDate={currentDate} founderName="Matías" initialPreferences={founderWorkspace} pendingTasks={taskSummary.pending} todayEvents={projects.filter(project=>project.event.date===today).length} values={workspaceValues}/></div>;
}

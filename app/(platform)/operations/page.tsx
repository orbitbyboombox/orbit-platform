...
type PlanningEvent = {
  ...
  booths: number;
  roles: {
    operator: PlanningRole;
    assembly: PlanningRole;
    disassembly: PlanningRole;
  };
  status: string;
  published: boolean;
...
export default async function OperationsPage() {
  ...
  const [
    ...
    client
      .from("staff")
      .select("id,first_name,last_name,status,capabilities")
      .eq("status", "ACTIVE")
      .is("deleted_at", null),
    client
    ...
    client
      ...
      .select(
        "project_id,asset_id,assignment_status,operational_assets(asset_type)",
      )
    ...
    client
      ...
      .select(
        "id,project_id,staff_id,total_internal_payment,paid_amount,settlement_status,operator_payment,assembly_payment,disassembly_payment,status,staff(first_name,last_name)",
      )
      .is("deleted_at", null)
      .eq("status", "CONFIRMED")
    ...
    client
      ...
      .select(
        "project_id,amount,paid_amount,outstanding_balance,effective_status,due_date,customer_type",
      ),
    client
      .from("expenses")
      .select("occurred_on,total,status")
      .is("deleted_at", null),
    client.from("customer_portal_tokens").select("project_id,revoked_at"),
    client
      .from("event_vehicle_assignments")
      .select("project_id,asset_id,status,operational_assets(asset_code)")
      .is("deleted_at", null),
  ...
  const assignments: ProductionAssignment[] = (
    ...
    status: item.status,
    resources: (item.resources ?? {}) as Record<string, unknown>,
  }));
  const staff = staffResult.data ?? [];
  const assets = assetsResult.data ?? [];
  const assetAssignments = (assetAssignmentsResult.data ??
    []) as unknown as Array<{
    project_id: string;
    asset_id: string;
    assignment_status: string;
    operational_assets: { asset_type: string } | null;
  }>;
  const agreements = agreementsResult.data ?? [];
  const evidence = new Set(
    (evidenceResult.data ?? []).map((item) => item.agreement_id),
  );
  ...
  const financeByProject = new Map(
    (rawProjectsResult.data ?? []).map((item) => [
      item.id,
      (item.finance ?? {}) as Record<string, unknown>,
    ]),
  );
  const certificationCustomers = new Set(
    (customersResult.data ?? [])
      .filter((item) => {
        const metadata = (item.metadata ?? {}) as Record<string, unknown>;
        return (
          metadata.record_type === "SYSTEM_CERTIFICATION" ||
          metadata.recordType === "QA_OPERATIONAL_CERTIFICATION" ||
          typeof metadata.validation === "string"
        );
      })
      .map((item) => item.id),
  );
  const certificationProjects = new Set(
    (rawProjectsResult.data ?? [])
      .filter((item) => certificationCustomers.has(item.customer_id))
      .map((item) => item.id),
  );
  const projects = allProjects.filter(
    (project) => !certificationProjects.has(project.id),
  );

  const readiness: CommandCenterProjectReadiness[] = projects
    ...
    .map((project) => {
      ...
      const projectAssets = assetAssignments.filter(
        (item) => item.project_id === project.id,
      );
      ...
      const state = (condition: boolean, attention = false) =>
        condition
          ? ("READY" as const)
          : attention
            ? ("ATTENTION" as const)
      ...
      return {
        ...
        statuses: [
          ...
          {
            label: "Cotización",
            state: state(quotation?.status === "ACCEPTED"),
          },
          { label: "Acuerdo", state: state(agreement?.status === "SIGNED") },
          {
            label: "Firma",
            state: state(Boolean(agreement && evidence.has(agreement.id))),
          },
          {
            label: "Calendar",
            state: state(
          ...
          },
          {
            ...
            state: state(
              projectAssets.some(
                (item) => item.operational_assets?.asset_type === "TOTEM",
              ),
            ),
          },
          {
            ...
            state: state(
              projectAssets.some(
                (item) => item.operational_assets?.asset_type === "CASE",
              ),
            ),
          },
          {
            label: "Payroll",
            state: state(
              payroll.some((item) => item.project_id === project.id),
              true,
            ),
          },
          {
            label: "Profit",
            state: state(
              profit.some((item) => item.projectId === project.id),
              true,
            ),
          },
          {
            label: "Timeline",
            state: state(
              timeline.some((item) => item.project_id === project.id),
              true,
            ),
          },
        ],
  ...
  const assignedToday = new Set(
    assignments
      .filter(
        (item) =>
          todayProjectIds.has(item.projectId) &&
          item.type === "OPERATOR" &&
          item.status !== "REJECTED",
      )
  ...
  const availableOperators = staff.filter(
    (member) =>
      Array.isArray(member.capabilities) &&
      member.capabilities.includes("OPERATOR") &&
      !assignedToday.has(member.id),
  ).length;
  const availableTotems = assets.filter(
    (asset) => asset.asset_type === "TOTEM" && asset.status === "AVAILABLE",
  ).length;
  const availableCases = assets.filter(
    (asset) => asset.asset_type === "CASE" && asset.status === "AVAILABLE",
  ).length;
  const availablePrinters = assets.filter(
    (asset) => asset.asset_type === "PRINTER" && asset.status === "AVAILABLE",
  ).length;
  const availableCameras = assets.filter(
    (asset) => asset.asset_type === "CAMERA" && asset.status === "AVAILABLE",
  ).length;
  ...
  const monthKey = new Intl.DateTimeFormat("en-CA", {
    ...
    .format(new Date())
    .slice(0, 7);
  const monthlyRevenue = financialTruth
    .filter(
      (item) =>
        item.status === "CONFIRMED" && item.eventDate?.startsWith(monthKey),
    )
    .reduce((sum, item) => sum + item.revenue, 0);
  const monthlyExpenses = (expensesResult.data ?? [])
    .filter((item) => item.occurred_on?.startsWith(monthKey))
    .reduce((sum, item) => sum + Number(item.total ?? 0), 0);
  const availableVehicles = assets.filter(
    (asset) => asset.asset_type === "VEHICLE" && asset.status === "AVAILABLE",
  ).length;
  const activeAssignments = assignments.filter(
    (item) => !["CANCELLED", "REJECTED", "COMPLETED"].includes(item.status),
  );
  const countBySkill = (skill: string) =>
    staff
      .map((member) => ({
        member,
  ...
  const topOperator = countBySkill("OPERATOR"),
    topAssembly = countBySkill("ASSEMBLY");
  const totalStaffCost = payroll
    .filter((item) => item.status !== "CANCELLED")
    .reduce((sum, item) => sum + Number(item.total_internal_payment), 0);
  ...
  const staffIntelligence = {
    mostActiveOperator: topOperator?.count
      ? `${topOperator.member.first_name} ${topOperator.member.last_name}`
      : "Sin datos",
    mostActiveAssembly: topAssembly?.count
      ? `${topAssembly.member.first_name} ${topAssembly.member.last_name}`
      : "Sin datos",
    totalStaffCost,
    averageCostPerEvent: paidProjects.size
      ? totalStaffCost / paidProjects.size
      : 0,
    upcomingAssignments: activeAssignments.length,
  };
  ...
  const confirmed = financialTruth.filter(
    (row) => row.status === "CONFIRMED" && projectMap.has(row.projectId),
  );
  const todayRevenue = confirmed
    .filter((row) => row.eventDate === today)
    .reduce((sum, row) => sum + row.revenue, 0);
  const monthStatements = confirmed.filter((row) =>
    row.eventDate?.startsWith(monthKey),
  );
  const monthRevenue = monthStatements.reduce(
    (sum, row) => sum + row.revenue,
    0,
  );
  const estimatedProfit = monthStatements.reduce(
    (sum, row) => sum + row.revenue - row.estimatedCost,
    0,
  );
  const realProfit = monthStatements.reduce(
    (sum, row) => sum + row.netProfit,
    0,
  );
  const totalMonthRevenue = monthStatements.reduce(
    (sum, row) => sum + row.revenue,
    0,
  );
  const grossMargin = totalMonthRevenue
    ? (estimatedProfit / totalMonthRevenue) * 100
    : 0;
  const netMargin = totalMonthRevenue
    ? (realProfit / totalMonthRevenue) * 100
    : 0;
    ...
    (payrollResult.data ?? [])
      ...
      .reduce(
        (sum, row) =>
          ...
          Math.max(
            0,
            Number(row.total_internal_payment) - Number(row.paid_amount),
          ),
        0,
      );
  const cashPosition =
    confirmed.reduce((sum, row) => sum + row.paidAmount, 0) -
    (expensesResult.data ?? [])
  ...
  const projectScore = confirmed.map((row) => ({
    row,
    project: projectMap.get(row.projectId)!,
    profit: row.netProfit,
    margin: row.netMargin,
  }));
  const best = [...projectScore].sort((a, b) => b.profit - a.profit)[0],
    lowest = [...projectScore].sort((a, b) => a.margin - b.margin)[0];
  const groupProfit = (key: (p: (typeof projects)[number]) => string) => {
    const grouped = new Map<string, number>();
    for (const item of projectScore) {
      const label = key(item.project) || "Sin registro";
      grouped.set(label, (grouped.get(label) ?? 0) + item.profit);
    }
    return [...grouped].sort((a, b) => b[1] - a[1])[0];
  };
  const serviceWinner = groupProfit((project) => project.services.join(" + ")),
    municipalityWinner = groupProfit((project) => project.event.city);
  const costSum = (key: string) =>
    confirmed.reduce(
      (sum, row) => sum + Number(row.costBreakdown[key] ?? 0),
      0,
    );
  const totalReal = confirmed.reduce((sum, row) => sum + row.realCost, 0);
  const activeInvoices = receivables.filter(
    (row) => !["PAID", "CANCELLED"].includes(row.effective_status),
  );
  const overdue = activeInvoices.filter(
    (row) => row.effective_status === "OVERDUE",
  );
  const unsigned = (agreementsResult.data ?? []).filter(
    (row) => row.status !== "SIGNED",
  ).length;
  const corporatePending = activeInvoices.filter(
    (row) => row.customer_type === "CORPORATE",
  ).length;
  const missingStaff = projects.filter(
    (project) =>
      project.event.date >= today &&
      !assignments.some(
        (item) =>
          item.projectId === project.id &&
  ...
  const truthIds = new Set(financialTruth.map((row) => row.projectId));
  const missingCosts = projects.filter(
    (project) => project.event.date >= today && !truthIds.has(project.id),
  ).length;
  const lowMargin = projectScore.filter((item) => item.margin < 20).length;
  const vehicleEvents = vehicleEventsResult.data ?? [];
  const vehicleCounts = new Map<string, number>();
  for (const row of vehicleEvents) {
    const asset = Array.isArray(row.operational_assets)
      ? row.operational_assets[0]
      : row.operational_assets;
    const key = asset?.asset_code ?? "Sin vehículo";
    vehicleCounts.set(key, (vehicleCounts.get(key) ?? 0) + 1);
  }
  const topVehicle = [...vehicleCounts].sort((a, b) => b[1] - a[1])[0];
  const metric = (
    label: string,
    value: string,
    detail: string,
  ...
  const controlData: ExecutiveControlData = {
    generatedAt: new Date().toISOString(),
    top: [
      metric(
        "Ingresos de hoy",
        money(todayRevenue),
        "Desde Business Truth",
        "/projects?date=today",
      ),
      metric(
        "Ingresos mensuales",
        money(monthRevenue),
        "Desde Business Truth",
        "/projects?period=month",
      ),
      metric(
        "Cuentas por cobrar",
        money(accountsReceivable),
        `${activeInvoices.length} documentos abiertos`,
        "/finance/receivables",
      ),
      metric(
        "Cuentas por pagar",
        money(accountsPayable),
        "Gastos y pagos de Staff pendientes",
        "/finance/expenses",
      ),
      metric(
        "Profit estimado",
        money(estimatedProfit),
        "Ingreso menos costo estimado",
    ...
    sections: [
      {
        ...
        metrics: [
          metric(
            "Eventos de hoy",
            String(projects.filter((p) => p.event.date === today).length),
            "Agenda actual",
            "/projects?date=today",
          ),
          metric(
            "Eventos de mañana",
            String(projects.filter((p) => p.event.date === tomorrow).length),
            "Próxima jornada",
            "/projects?date=tomorrow",
          ),
          metric(
            "Próximos 15 días",
            String(next15Events),
          ...
          metric(
            ...
            String(unsigned),
            "Esperando firma",
            "/projects?contract=pending",
          ),
          metric(
            "Crédito empresa",
            String(corporatePending),
            "Facturas corporativas abiertas",
            "/finance/receivables?customer=corporate",
          ),
          metric(
            "Pagos pendientes",
            String(activeInvoices.length),
      ...
      },
      {
        ...
        metrics: [
          metric(
            "Evento más rentable",
            best?.project.name ?? "Sin datos",
            best ? money(best.profit) : "—",
            best ? `/projects/${best.project.id}#profitability` : "/projects",
          ),
          metric(
            "Servicio más rentable",
            serviceWinner?.[0] ?? "Sin datos",
            serviceWinner ? money(serviceWinner[1]) : "—",
            "/projects?view=profitability",
          ),
          metric(
            "Comuna más rentable",
            municipalityWinner?.[0] ?? "Sin datos",
            municipalityWinner ? money(municipalityWinner[1]) : "—",
            "/projects?view=profitability",
          ),
          metric(
            "Menor margen",
            lowest?.project.name ?? "Sin datos",
            lowest ? pct(lowest.margin) : "—",
            lowest
              ? `/projects/${lowest.project.id}#profitability`
              : "/projects",
          ),
          metric(
            "Margen promedio",
            pct(
              projectScore.length
                ? projectScore.reduce((sum, item) => sum + item.margin, 0) /
                    projectScore.length
                : 0,
            ),
            "Eventos con estado financiero",
            "/projects?view=profitability",
          ),
        ],
      },
      {
        ...
        metrics: [
          ...
          metric(
            "Operadores",
            money(costSum("operator")),
            "Costo vigente",
            "/projects?cost=operator",
          ),
          metric(
            "Montaje",
            money(costSum("assembly")),
            "Montaje y desmontaje",
            "/projects?cost=assembly",
          ),
          metric(
            "Transporte",
            money(costSum("transport")),
            "Costo vigente",
            "/projects?cost=transport",
          ),
          metric(
            "Promedio por evento",
            money(confirmed.length ? totalReal / confirmed.length : 0),
            "Costo real promedio",
            "/projects?view=profitability",
          ),
        ],
      },
      {
        ...
        metrics: [
              ...
              (payrollResult.data ?? [])
                ...
                .reduce(
                  (sum, row) =>
                    ...
                    Math.max(
                      0,
                      Number(row.total_internal_payment) -
          ...
          metric(
            "Operador más activo",
            staffIntelligence.mostActiveOperator,
            "Eventos completados",
            "/resources/staff",
          ),
        ],
      },
      {
        ...
        metrics: [
          metric(
            ...
            topVehicle?.[0] ?? "Sin datos",
            topVehicle ? `${topVehicle[1]} eventos` : "—",
            "/resources?tab=fleet",
          ),
          metric(
            "Costo combustible",
          ...
          metric(
            "Próxima mantención",
            "Preparado",
            "Disponible para futura configuración",
            "/resources?tab=fleet",
          ),
        ],
      },
      {
        ...
        metrics: [
          metric(
            "Contratos firmados",
            String(
              (agreementsResult.data ?? []).filter(
                (row) => row.status === "SIGNED",
              ).length,
            ),
            "Documentos oficiales firmados",
            "/projects?contract=signed",
          ),
          metric(
            "Documentos comerciales",
            String(
              (agreementsResult.data ?? []).filter(
                (row) => row.status === "COMMERCIAL_DOCUMENT",
              ).length,
            ),
            "Sin firma requerida",
            "/projects?document=commercial",
          ),
          metric(
            "Portales activos",
            String(
              (portalResult.data ?? []).filter((row) => !row.revoked_at).length,
            ),
            "Acceso cliente disponible",
            "/projects?portal=active",
          ),
        ],
      },
    ],
    alerts: [
      ...
      ...(corporatePending
        ? [
            metric(
              "Facturas corporativas pendientes",
              String(corporatePending),
              "Crédito abierto",
              "/finance/receivables?customer=corporate",
              "warning",
      ...
      ...(missingStaff
        ? [
            metric(
              "Eventos sin Staff",
              String(missingStaff),
              "Asignación operacional pendiente",
              "/projects?staff=missing",
              "danger",
            ),
          ]
        : []),
      ...(missingCosts
        ? [
            metric(
              "Costos operacionales faltantes",
              String(missingCosts),
              "Sin estado financiero",
              "/projects?costs=missing",
              "warning",
            ),
          ]
        : []),
  ...
  const monthlyPersonnelCost = monthStatements.reduce(
    (sum, row) => sum + row.personnelCost,
    0,
  );
  const monthlyOperationalResourcesCost = monthStatements.reduce(
    (sum, row) => sum + row.operationalResourcesCost,
    0,
  );
  const monthlyTotalOperationalCost = monthStatements.reduce(
    (sum, row) => sum + row.totalOperationalCost,
    0,
  );
  controlData.top.unshift(
    ...
    metric(
      "Costo de personal",
      money(monthlyPersonnelCost),
      "Operador + montaje + desmontaje",
      "/projects?cost=personnel",
    ),
    metric(
      "Recursos operacionales",
      money(monthlyOperationalResourcesCost),
      "Cost Master y ajustes reales",
      "/projects?cost=resources",
    ),
    metric(
      "Costo operacional total",
      money(monthlyTotalOperationalCost),
      "Personal + recursos operacionales",
      "/projects?view=profitability",
    ),
    metric(
      "Profit",
      money(realProfit),
      "Ingreso menos costo operacional total",
      "/projects?view=profitability",
    ),
    metric(
      "Margen",
  ...
  planningEndDate.setUTCDate(planningEndDate.getUTCDate() + 15);
  ...
  const officialEventMap = new Map(
    (rawProjectsResult.data ?? []).map((row) => [row.id, row]),
  );
  const planningEvents: PlanningEvent[] = projects
    .filter((project) =>
      Boolean(
        project.event.date &&
          project.event.date >= today &&
          project.event.date <= planningEnd,
      ),
    )
    .map((project) => {
      const official = officialEventMap.get(project.id);
      const services = official
        ? Array.isArray(official.project_services)
          ? official.project_services
          : []
        : [];
      const operations = (official?.operations ?? {}) as Record<
        string,
        unknown
      >;
      const customer = official
        ? Array.isArray(official.customers)
          ? official.customers[0]
          : official.customers
        : null;
      ...
      const findRole = (
        ...
        const assignment = projectAssignments.find(
          (item) => item.assignment_type === code,
        );
        const person = assignment?.staff
          ? Array.isArray(assignment.staff)
            ...
            : assignment.staff
          : null;
        const payment = (payrollResult.data ?? []).find(
          (item) =>
            item.project_id === project.id &&
            item.staff_id === assignment?.staff_id &&
            item.status !== "CANCELLED",
        );
        const stored =
          code === "OPERATOR"
            ? payment?.operator_payment
            : code === "ASSEMBLY"
              ? payment?.assembly_payment
              : payment?.disassembly_payment;
        return {
          code,
          label,
          pay: Number(stored ?? 0),
          ...
          assignee: person ? `${person.first_name} ${person.last_name}` : null,
      ...
      return {
        ...
        customer: customer?.full_name ?? project.client.name,
        service:
          services
        ...
        venue: String(
          operations.venue ?? operations.venueName ?? official?.location ?? "",
        ),
        booths: Number(operations.booths ?? finance.booths ?? 1),
        roles: {
          operator: findRole("OPERATOR", "Operador"),
          assembly: findRole("ASSEMBLY", "Montaje"),
          disassembly: findRole("DISASSEMBLY", "Desmontaje"),
        },
  ...
  const pendingRequests: PlanningRequest[] = (
    ...
    return {
      ...
      customer: customer?.full_name ?? project?.name ?? "Evento",
      service:
        services
    ...
    <StaffOperationsView
      events={planningEvents.map((event) => ({
        ...
        assignments: [
          ...
          event.roles.disassembly,
        ]
          .filter((role) => role.assignee)
          .map((role) => `${role.label}: ${role.assignee}`),
        settlements: (payrollResult.data ?? [])
          .filter(
            (item) =>
              item.project_id === event.id && item.status !== "CANCELLED",
          )
          .map((item) => {
            const member = Array.isArray(item.staff)
              ? item.staff[0]
              : item.staff;
            return {
              id: item.id,
        ...
        requestCount: pendingRequests.filter(
          (request) =>
            (staffRequestsResult.data ?? []).find(

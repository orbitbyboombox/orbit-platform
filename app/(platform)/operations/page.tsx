...
export default async function OperationsPage() {
  ...
  return <div className="space-y-6"><OperationsStaffPortalControl events={planningEvents} requests={pendingRequests}/><OperationsPlanningBoard configuredPay={configuredPay} events={planningEvents} staff={staffOptions}/><FounderWorkspaceExperience currentDate={currentDate} founderName="Matías" initialPreferences={founderWorkspace} pendingTasks={taskSummary.pending} todayEvents={projects.filter(project=>project.event.date===today).length} values={workspaceValues}/></div>;
}

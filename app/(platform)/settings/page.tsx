...
export default async function SettingsPage() {
  ...
  const [communication, masterData, companySettings,modules,founderWorkspace,banking,{data:profitabilitySettings},{data:integrity},{data:diagnostics,error:diagnosticsError},{data:founderDeliveries,error:founderDeliveryError},{data:crmDiagnostics,error:crmDiagnosticsError}] = await Promise.all([loadCommunicationHubProjection(client), loadMasterData(client),loadCompanySettings(client),loadModuleStates(client),loadFounderWorkspace(client,auth.user.id),loadBankingReadModel(client),client.from("profitability_settings").select("high_margin_threshold,normal_margin_threshold").eq("settings_key","PRIMARY").single(),client.from("financial_integrity_status").select("integrity_percent,reservation_sync,finance_sync,dashboard_sync,business_intelligence_sync,reports_sync,affected_records,checked_at").eq("status_key","PRIMARY").single(),client.from("reservation_execution_diagnostics").select("reference,status,failed_step,exception_message,affected_record,suggested_fix,steps,created_at").order("created_at",{ascending:false}).limit(10),client.from("founder_notification_deliveries").select("id,project_id,recipient,attempt_number,status,provider_response,failure_reason,attempted_at,projects(name,orbit_event_id),customers(full_name)").order("attempted_at",{ascending:false}).limit(30),client.from("crm_profile_diagnostics").select("id,customer_name,module,failed_component,exception,suggested_cause,created_at").order("created_at",{ascending:false}).limit(20)]);
  if(diagnosticsError||founderDeliveryError||crmDiagnosticsError)throw diagnosticsError??founderDeliveryError??crmDiagnosticsError;
  ...
  return <PersonalWorkspaceSections moduleKey="SETTINGS" sections={[
    ...
    {key:"FINANCE_BANK_ACCOUNTS",label:"Finance · Cuentas Bancarias",content:<BankSettings accounts={banking.accounts} rules={banking.recurringRules}/>},

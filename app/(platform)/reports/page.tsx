...
export default async function ReportsPage(){
  ...
  const [customers,projects,services,quotations,quotationItems,assignments,payroll,staff,assets,assetAssignments,assetHistory,reviews,reviewStaff,profiles,receivables]=await Promise.all([
    ...
    client.from("staff_settlement_financials").select("project_id,staff_id,total_internal_payment:payroll_net,status:settlement_status").gt("payroll_net",0),
  ...
  const firstError=[customers,projects,services,quotations,quotationItems,assignments,payroll,staff,assets,assetAssignments,assetHistory,reviews,reviewStaff,profiles,receivables].find(result=>result.error)?.error; if(firstError)throw firstError;
  ...
  const dataset:BusinessIntelligenceDataset={
    ...
    quotationItems:quotationItems.data??[],profits:financialTruth.filter(item=>projectIds.has(item.projectId)&&item.status==="CONFIRMED").map(item=>({id:item.projectId,project_id:item.projectId,revenue:item.revenue,operational_cost:item.totalOperationalCost,gross_margin:item.grossProfit,gross_margin_percent:item.grossMargin,created_at:item.calculatedAt,status:item.status})),assignments:(assignments.data??[]).filter(item=>projectIds.has(item.project_id)),

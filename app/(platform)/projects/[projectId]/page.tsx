...
export default async function ProjectWorkspacePage({ params, searchParams }: ProjectWorkspacePageProps) {
  ...
  const [{ data: rawProject }, { data: agreement }, { data: assignments }, { data: documents }, { data: quotation }, { data: assets }, { data: assetAssignments }, { data: staff }, { data: operatorAssignments }, { data: calendarSync }, { data: driveSync }, { data: payroll }, { data: profit }, { data: invoice }, {data:checklist}, {data:estimatedCosts}, {data:realCostOverrides}, {data:profitabilityStatement}] = await Promise.all([
    ...
    client.from("financial_event_records").select("revenue,estimated_cost,real_cost,gross_profit,gross_margin,net_profit,net_margin,cost_breakdown,calculated_at").eq("project_id",projectId).maybeSingle(),
  ]);
  ...
  const event360 = {
    ...
    profitability:profitabilityStatement?(()=>{const revenue=Number(profitabilityStatement.revenue),estimated=Number(profitabilityStatement.estimated_cost),real=Number(profitabilityStatement.real_cost),margin=Number(profitabilityStatement.net_margin);return{revenue:{finalSalePrice:revenue},estimated:{total:estimated},real:{...(profitabilityStatement.cost_breakdown as Record<string,number>),total:real},profitability:{grossRevenue:revenue,estimatedCost:estimated,realCost:real,grossProfit:Number(profitabilityStatement.gross_profit),netProfit:Number(profitabilityStatement.net_profit),margin},variance:{amount:real-estimated,percentage:estimated?(real-estimated)/estimated*100:0,reason:"Business Truth Engine"},classification:(margin>=40?"HIGHLY_PROFITABLE":margin>=20?"NORMAL":"LOW_MARGIN") as "HIGHLY_PROFITABLE"|"NORMAL"|"LOW_MARGIN",createdAt:profitabilityStatement.calculated_at}})():undefined,

...
export default async function ProjectWorkspacePage({ params, searchParams }: ProjectWorkspacePageProps) {
  ...
  const{error:profitabilitySyncError}=await client.rpc("sync_event_profitability",{p_project_id:projectId});if(profitabilitySyncError)throw profitabilitySyncError;
  ...
  const [{ data: rawProject }, { data: agreement }, { data: assignments }, { data: documents }, { data: quotation }, { data: assets }, { data: assetAssignments }, { data: staff }, { data: operatorAssignments }, { data: calendarSync }, { data: driveSync }, { data: payroll }, { data: profit }, { data: invoice }, {data:checklist}, {data:estimatedCosts}, {data:realCostOverrides}, {data:profitabilityStatement}] = await Promise.all([
    ...
    client.from("event_profitability_statements").select("revenue,estimated_costs,real_costs,profitability,variance,performance,classification,created_at").eq("project_id",projectId).order("created_at",{ascending:false}).limit(1).maybeSingle(),
  ]);
  ...
  const event360 = {
    ...
    profitability:profitabilityStatement?{revenue:profitabilityStatement.revenue as Record<string,number|string>,estimated:profitabilityStatement.estimated_costs as Record<string,number>,real:profitabilityStatement.real_costs as Record<string,number>,profitability:profitabilityStatement.profitability as Record<string,number>,variance:profitabilityStatement.variance as {amount:number;percentage:number;reason:string},performance:profitabilityStatement.performance as Record<string,number>,classification:profitabilityStatement.classification as "HIGHLY_PROFITABLE"|"NORMAL"|"LOW_MARGIN",createdAt:profitabilityStatement.created_at}:undefined,

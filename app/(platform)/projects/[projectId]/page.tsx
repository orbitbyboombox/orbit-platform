...
export default async function ProjectWorkspacePage({ params, searchParams }: ProjectWorkspacePageProps) {
  ...
  const timeline = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(projectId)
    ...
    : [];
  const [{ data: rawProject }, { data: agreement }, { data: assignments }, { data: documents }, { data: quotation }, { data: assets }, { data: assetAssignments }, { data: staff }, { data: operatorAssignments }, { data: calendarSync }, { data: driveSync }, { data: payroll }, { data: profit }, { data: invoice }, {data:checklist}, {data:estimatedCosts}, {data:realCostOverrides}, {data:profitabilityStatement}] = await Promise.all([
    ...
    client.from("drive_sync").select("id,status,destination_key,external_folder_id,last_synced_at").eq("project_id", projectId),
    ...
    client.from("estimated_cost_sheets").select("status,paper,operator,assembly,disassembly,fuel,transport,scrapbook,magnets,pens,double_sided_tape,other_configured,total,calculated_at").eq("project_id",projectId).maybeSingle(),
    client.from("financial_cost_overrides").select("category,estimated_value,edited_value,reason,created_at").eq("project_id",projectId).order("created_at",{ascending:false}),
    client.from("financial_event_records").select("revenue,estimated_cost,real_cost,personnel_cost,operational_resources_cost,total_operational_cost,gross_profit,gross_margin,net_profit,net_margin,cost_breakdown,calculated_at").eq("project_id",projectId).maybeSingle(),
  ]);
  ...
  const event360 = {
    ...
    google: { calendarStatus: calendarSync?.status ?? "PENDING", calendarUrl: calendarSync?.external_url ?? undefined, driveStatus: (driveSync??[]).some(item=>item.status==="ERROR")?"ERROR":(driveSync??[]).some(item=>item.status==="SYNCING")?"SYNCING":(driveSync??[]).length?"CONNECTED":"PENDING", driveLastSyncedAt:(driveSync??[]).map(item=>item.last_synced_at).filter((value):value is string=>Boolean(value)).sort().at(-1), driveUrl: (()=>{const folder=(driveSync??[]).find(item=>item.destination_key.split("/").length===4&&item.external_folder_id);return folder?.external_folder_id?`https://drive.google.com/drive/folders/${folder.external_folder_id}`:undefined})(), gmailStatus: communications?.find((item) => item.channel === "GMAIL")?.status ?? "PENDING", gmailThread: communications?.find((item) => item.channel === "GMAIL")?.thread_key },
    ...
    profit: realCost,
    estimatedCosts: estimatedCosts ? {status:estimatedCosts.status,paper:Number(estimatedCosts.paper),operator:Number(estimatedCosts.operator),assembly:Number(estimatedCosts.assembly),disassembly:Number(estimatedCosts.disassembly),fuel:Number(estimatedCosts.fuel),transport:Number(estimatedCosts.transport),scrapbook:Number(estimatedCosts.scrapbook),magnets:Number(estimatedCosts.magnets),pens:Number(estimatedCosts.pens),doubleSidedTape:Number(estimatedCosts.double_sided_tape),other:Number(estimatedCosts.other_configured),total:Number(estimatedCosts.total),calculatedAt:estimatedCosts.calculated_at} : undefined,
    realCosts: estimatedCosts ? (()=>{const definitions=[['OPERATOR','Operador',Number(estimatedCosts.operator)],['ASSEMBLY','Montaje',Number(estimatedCosts.assembly)],['DISASSEMBLY','Desmontaje',Number(estimatedCosts.disassembly)],['FUEL','Combustible',Number(estimatedCosts.fuel)],['TRANSPORT','Transporte',Number(estimatedCosts.transport)],['PARKING','Estacionamiento',0],['TOLLS','Peajes',0],['MEALS','Alimentación',0],['HOTEL','Hotel',0],['SCRAPBOOK','Scrapbook',Number(estimatedCosts.scrapbook)],['MAGNETS','Imanes',Number(estimatedCosts.magnets)],['OTHER_OPERATIONAL','Otros costos operacionales',Number(estimatedCosts.other_configured)]] as const;const latest=new Map<string,{category:string;edited_value:number;reason:string;created_at:string}>();for(const row of realCostOverrides??[])if(!latest.has(row.category))latest.set(row.category,row);const items=definitions.map(([category,label,estimated])=>{const row=latest.get(category);return{category,label,estimated,real:row?Number(row.edited_value):estimated,reason:row?.reason,updatedAt:row?.created_at}});return{paper:Number(estimatedCosts.paper),items,estimatedTotal:Number(estimatedCosts.total),realTotal:Number(estimatedCosts.paper)+items.reduce((sum,item)=>sum+item.real,0)}})() : undefined,
    profitability:profitabilityStatement?(()=>{const revenue=Number(profitabilityStatement.revenue),estimated=Number(profitabilityStatement.estimated_cost),real=Number(profitabilityStatement.real_cost),personnel=Number(profitabilityStatement.personnel_cost),resources=Number(profitabilityStatement.operational_resources_cost),margin=Number(profitabilityStatement.net_margin);return{revenue:{finalSalePrice:revenue},estimated:{total:estimated},real:{...(profitabilityStatement.cost_breakdown as Record<string,number>),personnelCost:personnel,operationalResourcesCost:resources,totalOperationalCost:Number(profitabilityStatement.total_operational_cost),total:real},profitability:{grossRevenue:revenue,estimatedCost:estimated,realCost:real,operationalCost:real,grossProfit:Number(profitabilityStatement.gross_profit),netProfit:Number(profitabilityStatement.net_profit),margin},variance:{amount:real-estimated,percentage:estimated?(real-estimated)/estimated*100:0,reason:"Motor de Costos Operacionales"},classification:(margin>=40?"HIGHLY_PROFITABLE":margin>=20?"NORMAL":"LOW_MARGIN") as "HIGHLY_PROFITABLE"|"NORMAL"|"LOW_MARGIN",createdAt:profitabilityStatement.calculated_at}})():undefined,

...
export default async function ProjectWorkspacePage({ params, searchParams }: ProjectWorkspacePageProps) {
  ...
  const timeline = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(projectId)
    ...
    : [];
  const [{ data: rawProject }, { data: agreement }, { data: assignments }, { data: documents }, { data: quotation }, { data: assets }, { data: assetAssignments }, { data: staff }, { data: operatorAssignments }, { data: calendarSync }, { data: driveSync }, { data: payroll }, { data: profit }, { data: invoice }, {data:checklist}, {data:estimatedCosts}, {data:realCostOverrides}] = await Promise.all([

...
export default async function CashFlowPage(){
  ...
  for(const expense of expensesResult.data??[]){let metadata:Record<string,unknown>={};try{metadata=JSON.parse(expense.approval_reason??"{}");}catch{}const paid=expense.status==="PAID";transactions.push({id:expense.id,kind:paid?"OUTGOING":"PENDING_PAYMENT",date:expense.occurred_on,amount:Number(expense.total),title:expense.supplier||"Gasto BOOMBOX",detail:String(metadata.description??expense.category),href:`/finance/expenses?expense=${expense.id}`,source:expense.category});}
  const months=new Map((staffMonthsResult.data??[]).map(item=>[item.id,item]));
  for(const advance of staffAdvancesResult.data??[]){const month=months.get(advance.payment_month_id);const member=relation<{first_name?:string;last_name?:string}>(month?.staff);const name=`${member?.first_name??""} ${member?.last_name??""}`.trim();transactions.push({id:advance.id,kind:"OUTGOING",date:advance.created_at.slice(0,10),amount:Number(advance.amount),title:"Anticipo de Staff",detail:name||"Staff BOOMBOX",href:"/resources/staff",source:"Staff"});}
  for(const month of staffMonthsResult.data??[]){const member=relation<{first_name?:string;last_name?:string}>(month.staff);const name=`${member?.first_name??""} ${member?.last_name??""}`.trim();const paid=Number(month.paid_amount);if(paid>0)transactions.push({id:`staff-paid-${month.id}`,kind:"OUTGOING",date:month.updated_at.slice(0,10),amount:paid,title:"Pago de Staff",detail:`${name||"Staff BOOMBOX"} · ${month.month.slice(0,7)}`,href:"/resources/staff",source:"Staff"});}

...
export default async function FinancePage() {
  ...
  return <PersonalWorkspaceSections moduleKey="FINANCE" sections={[
    { key: "FINANCE_DASHBOARD", label: "Dashboard financiero", content: <FinancialDashboard data={dashboard}/> },
    { key: "ACCOUNTS_RECEIVABLE", label: "Cuentas por cobrar", content: <Link className="group flex items-center justify-between gap-4 rounded-2xl border border-brand/25 bg-brand/5 p-5 transition hover:border-brand/50" href="/finance/receivables"><div className="flex items-center gap-4"><span className="grid size-11 place-items-center rounded-xl bg-brand/10 text-brand"><ReceiptText className="size-5"/></span><div><p className="font-semibold">Cuentas por Cobrar</p><p className="mt-1 text-sm text-muted">Facturas, vencimientos, pagos y antigüedad de saldos.</p></div></div><ArrowRight className="size-5 text-muted transition group-hover:translate-x-1 group-hover:text-brand"/></Link> },
    { key: "EVENT_PROFITABILITY", label: "Rentabilidad de eventos", content: <div className="scroll-mt-24" id="profitability"><ProfitabilityExperience events={events} insights={insights} recommendation={recommendation} supplies={supplies}/></div> },
  ]}/>;
}

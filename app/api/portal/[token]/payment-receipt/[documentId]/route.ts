...
export async function GET(request: Request, { params }: { params: Promise<{ token: string; documentId: string }> }) {
  ...
  const listed = portal.documents.find((item) => item.id === documentId && item.document_type === "PAYMENT_RECEIPT");
  if (!listed) return NextResponse.json({ message: "El comprobante no está disponible." }, { status: 404 });
  const admin = createAdminClient();
  const { data: document } = await admin.from("documents").select("storage_bucket,storage_path").eq("id", documentId).eq("project_id", portal.access.project_id).eq("document_type", "PAYMENT_RECEIPT").is("deleted_at", null).single();
  ...
  const { data, error } = await admin.storage.from("orbit-documents").download(document.storage_path);

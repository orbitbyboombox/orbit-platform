...
export async function POST(request:NextRequest,{params}:{params:Promise<{token:string}>}){
  ...
  try{
    ...
    const admin=createAdminClient();const {data:folder}=await admin.from("drive_sync").select("external_folder_id").eq("project_id",portal.access.project_id).ilike("destination_key","%04_Diseños").not("external_folder_id","is",null).limit(1).maybeSingle();

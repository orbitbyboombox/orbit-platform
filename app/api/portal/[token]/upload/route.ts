...
export async function POST(request:NextRequest,{params}:{params:Promise<{token:string}>}){
  ...
  try{
    ...
    const admin=createAdminClient();const customer=Array.isArray(portal.project.customers)?portal.project.customers[0]:portal.project.customers;if(!customer?.full_name)throw new Error("No fue posible identificar al cliente.");
    const uploaded=await uploadReservationDocumentToDrive({client:admin,projectId:portal.access.project_id,customerName:customer.full_name,eventDate:portal.project.event_date,kind:"DESIGN",name:file.name,mimeType:file.type||"application/octet-stream",bytes:new Uint8Array(await file.arrayBuffer())});const correlationId=crypto.randomUUID();
    ...
    const {error:timelineError}=await admin.from("timeline_events").insert({customer_id:portal.access.customer_id,project_id:portal.access.project_id,event_type:"CUSTOMER_DESIGN_UPLOADED",title:"Archivo de diseño recibido.",description:message,orbit_event_id:portal.project.orbit_event_id,actor_label:"Cliente",source:"Customer",action:"CUSTOMER_DESIGN_UPLOADED",entity_type:"CustomerPortalUpload",entity_id:record.id,human_message:"El cliente compartió material para el diseño.",correlation_id:correlationId});if(timelineError)throw timelineError;destination.searchParams.set("upload","success");

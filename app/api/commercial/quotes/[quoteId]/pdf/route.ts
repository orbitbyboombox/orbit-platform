  ...
  try {
    ...
    const [{ data: quote, error }, company] = await Promise.all([
      client
        .from("quotations")
        .select(
          "quotation_number,issue_date,expiration_date,customer_snapshot,commercial_snapshot,quotation_items(description,label,quantity,quoted_price,unit_price,total,display_order)",
        )
        .eq("id", quoteId)
        .single(),
    ...
    const snapshot = (quote.commercial_snapshot ?? {}) as Record<
      string,
      unknown
    >;

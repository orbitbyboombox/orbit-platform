...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    let window: { startAt: string; endAt: string };
    try {
      window = normalizeEventWindow({ eventDate: String(input.eventDate ?? ""), serviceStart: String(input.serviceStart ?? ""), serviceEnd: typeof input.serviceEnd === "string" ? input.serviceEnd : undefined, durationHours: typeof input.durationHours === "number" ? input.durationHours : undefined });
    } catch (error) {
      if (error instanceof BookingTimeInvalidError) return NextResponse.json({ ok: false, status: "BLOCKED", reasonCode: error.code, message: error.message }, { status: 400 });
      throw error;
    }
    ...
    const { data, error } = await admin.rpc("preflight_public_booking_capacity", { p_booking_token: token, p_service_codes: serviceCodes, p_event_type: typeof input.eventType === "string" ? input.eventType.slice(0, 80) : null, p_event_date: typeof input.eventDate === "string" ? input.eventDate : null, p_service_start: window.startAt, p_service_end: window.endAt, p_address: typeof input.address === "string" ? input.address.slice(0, 240) : "", p_city: typeof input.city === "string" ? input.city.slice(0, 120) : "", p_shell: input.shell === "WHITE" || input.shell === "BLACK" ? input.shell : null });
    if (error) {

...
create unique index if not exists automatic_booking_active_email_idx
  on public.automatic_booking_invitations(lower(customer_email))
  where consumed_at is null and status in ('SENT','OPENED','PROCESSING');

drop trigger if exists automatic_booking_invitations_audit on public.automatic_booking_invitations;
create trigger automatic_booking_invitations_audit after insert or update or delete on public.automatic_booking_invitations

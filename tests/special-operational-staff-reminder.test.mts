...
import {
  chileTomorrow, isChileD1ReminderWindow, isSpecialOperationalStaffId,
  operationalD1Candidates, operationalReminderCorrelation, specialD1Candidates,
  specialReminderCorrelation, SPECIAL_OPERATIONAL_STAFF_IDS,
  SPECIAL_OPERATIONAL_STAFF_MESSAGE, type OperationalReminderAssignment,
...
test("global D-1 groups every Staff person/event and merges roles",()=>{
  const row=(staffId:string,role:string):OperationalReminderAssignment=>({...base(staffId),firstName:"Staff",eventName:"Evento Premium",eventTime:"20:30:00",location:"Colina",role,customerId:"customer-1"});
  const candidates=operationalD1Candidates([row("normal-1","OPERATOR"),row("normal-1","ASSEMBLY"),row(jose,"OPERATOR")],tomorrow);
  assert.equal(candidates.length,2);
  assert.deepEqual(candidates[0].roles,["OPERATOR","ASSEMBLY"]);
  assert.equal(operationalReminderCorrelation("event-1","normal-1"),"staff-d1:event-1:normal-1");
  assert.equal(operationalReminderCorrelation("event-1",jose),specialReminderCorrelation("event-1",jose));
});
test("D-1 delivery window prefers 19:00 and permits only the 20:00 fallback",()=>{
  ...
  assert.equal(isChileD1ReminderWindow(new Date("2026-09-16T23:05:00Z")),true); // delayed 20:00 fallback
  assert.equal(isChileD1ReminderWindow(new Date("2026-09-16T23:30:00Z")),false);
  assert.equal(isChileD1ReminderWindow(new Date("2026-09-17T00:00:00Z")),false);
});
test("premium D-1 includes event truth, roles, address, CTA and BOOMBOX styling",()=>{
  const email=buildStaffD1ReminderEmail({appUrl:"https://orbit.boom-box.cl",firstName:"Sebastián",eventName:"Evento corporativo",eventDate:tomorrow,roles:["OPERATOR","ASSEMBLY"],eventTime:"20:30:00",location:"Puerta Oriente 361"});
...
test("all email entry points use the ID policy and D-1 claims are person/event scoped",()=>{
  ...
  assert.match(cron,/operationalReminderCorrelation\([\s\S]*candidate\.projectId,[\s\S]*candidate\.staffId/);
  ...
  assert.match(cron,/\.contains\("metadata",\s*\{\s*hours_before:\s*24,\s*email_status:\s*"SENT"\s*\}\)/);

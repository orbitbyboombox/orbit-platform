...
test("Founder Command Center exposes the canonical expense action", () => {
  assert.match(founder, /action\.new_expense/);
  assert.match(founder, /action\.new_customer/);
  assert.match(founder, /action\.new_reservation/);
  assert.match(founder, /action\.quote/);
...
test("quick action reuses Expense Center without another form or endpoint", () => {
  assert.match(founder, /LiveExpenseCapture/);
  assert.match(founder, /\/finance\/expenses\?create=1/);
});

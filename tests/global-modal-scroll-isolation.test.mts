...
test("cash commitments use MobileDialog",()=>{assert.match(cash,/MobileDialog eyebrow="Detalle de transacciones"/);assert.match(cash,/open\("Compromisos de caja"/);assert.doesNotMatch(cash,/detail&&<div className="fixed inset-0/)});
test("Expense Center uses MobileDialog",()=>{assert.match(expense,/MobileDialog eyebrow="Centro de Gastos"/);assert.doesNotMatch(expense,/fixed inset-x-0 bottom-0/)});

...
test("Phase C.6 keeps the PDF event language in admin and Staff surfaces", () => {
  ...
  assert.match(list, /grid-cols-\[38px_3px_54px_72px_68px_55px_43px_14px\]/);
  assert.match(list, /h-12 w-1 self-center rounded-full/);
  assert.match(list, /min-h-\[72px\]/);
  assert.match(list, /overflow-x-clip/);
  ...
  assert.match(admin, /title="Event 360° \/ Resumen general" summary=\{`\$\{display\(status/);
  assert.match(admin, /detailOpen\.overview/);
  assert.match(admin, /Object\.fromEntries\(DETAIL_KEYS\.map/);

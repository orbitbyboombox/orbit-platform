...
test("derived overdue projection opens each exact invoice while historical reminders remain resolved",()=>{assert.match(groupedMigration,/notification_type in\('INVOICE_OVERDUE','INVOICE_DUE_TODAY','INVOICE_DUE_SOON'\)/);assert.match(repository,/"INVOICE_OVERDUE"/);assert.match(repository,/\/finance\/receivables\?invoice=/);assert.doesNotMatch(repository,/OVERDUE_INVOICE_GROUP/)});

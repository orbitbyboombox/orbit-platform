...
const middleware=readFileSync("middleware.ts","utf8"),receiptRoute=readFileSync("app/api/staff-monthly-accounts/[accountId]/receipt/route.ts","utf8");
...
test("monthly document routes bypass session refresh but keep internal ownership authorization",()=>{assert.match(middleware,/startsWith\("\/api\/staff-monthly-accounts\/"\)/);for(const route of [receiptRoute,boletaRoute,pdfRoute]){assert.match(route,/loadPortalSession\("STAFF"\)/);assert.match(route,/staff_id/);assert.match(route,/No autorizado/)} });
...
test("56 completed rows show status and no completion button",()=>{const source=readFileSync("features/staff-monthly-account/staff-monthly-account-panel.tsx","utf8");assert.match(source,/COMPLETADO ✓/);assert.match(source,/Pendientes de[\s\S]*completar/)})
test("57 completion confirmation preserves customer finance",()=>{const source=readFileSync("features/staff-monthly-account/staff-monthly-account-panel.tsx","utf8");assert.match(source,/saldo del cliente, cobranzas y pagos pendientes[\s\S]*continuarán activos/)})
test("58 settlement completion uses the canonical MobileDialog",()=>{const source=readFileSync("features/staff-monthly-account/staff-monthly-account-panel.tsx","utf8");assert.match(source,/MobileDialog/);assert.doesNotMatch(source,/window\.confirm/)})
test("59 completion dialog exposes explicit confirm and cancel actions",()=>{const source=readFileSync("features/staff-monthly-account/staff-monthly-account-panel.tsx","utf8");assert.match(source,/confirmCompletion/);assert.match(source,/Cancelar/);assert.match(source,/Marcar completado/)})
test("60 completion action has visible loading, success, and correlated failure states",()=>{const source=readFileSync("features/staff-monthly-account/staff-monthly-account-panel.tsx","utf8");assert.match(source,/Completando/);assert.match(source,/Evento marcado como completado/);assert.match(source,/Referencia/);assert.match(source,/CMP-/)})
test("61 MobileDialog cleans overlay and scroll-lock lifecycle",()=>{const source=readFileSync("components/ui/mobile-dialog.tsx","utf8");assert.match(source,/data-orbit-modal-overlay/);assert.match(source,/releaseLock/);assert.match(source,/document\.body/)})
test("62 completion refreshes the canonical monthly account projection",()=>{const source=readFileSync("features/staff-monthly-account/actions.ts","utf8");assert.match(source,/ensure_staff_monthly_account/);assert.match(source,/event_staff_payments/);assert.match(source,/revalidatePath\("\/resources\/staff"\)/)})
test("63 settlement UI derives both counters from the two canonical classification arrays",()=>{const source=readFileSync("features/staff-monthly-account/staff-monthly-account-panel.tsx","utf8");assert.match(source,/account\.calculation\.details\.length/);assert.match(source,/account\.calculation\.blockingEvents\.length/);assert.doesNotMatch(source,/useState<number>/)})
test("64 blocker projection excludes every canonical Completed spelling",()=>{const source=readFileSync("supabase/migrations/0203_staff_monthly_event_classification_reconciliation.sql","utf8");assert.match(source,/COMPLETED','COMPLETADO/);assert.match(source,/create or replace function public\.staff_monthly_blocking_events/)})
test("65 reconciliation migration has no record-specific identifiers",()=>{const source=readFileSync("supabase/migrations/0203_staff_monthly_event_classification_reconciliation.sql","utf8");assert.doesNotMatch(source,/ORB-2026|José|Nicolás|768712|632266|433421|078500/)})
test("66 month close has a server-side final settlement gate",()=>{const source=readFileSync("supabase/migrations/0204_staff_month_close_final_gate.sql","utf8");assert.match(source,/create or replace function public\.close_staff_month/);assert.match(source,/boleta_status in\('PENDING','RECEIVED','REJECTED'\)/);assert.match(source,/payment_status='READY_TO_PAY'/);assert.match(source,/payment_status<>'PAID'/);assert.match(source,/staff_monthly_blocking_events/)})
test("67 review-required settlement explains the blocker and stays payment-blocked in Founder UI",()=>{assert.match(panel,/Revisión Founder requerida/);assert.match(panel,/account\.reviewReason/);assert.match(panel,/approveMonthlySettlementReviewAction/);assert.match(panel,/READY_TO_PAY.*!account\.reviewRequired/)})
test("68 Founder review uses canonical eligibility override and refreshes the account",()=>{assert.match(actions,/override_staff_monthly_close_eligibility/);assert.match(actions,/ensure_staff_monthly_account/);assert.match(actions,/La liquidación mantiene bloqueos adicionales/)})

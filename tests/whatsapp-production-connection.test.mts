...
test("production health check is Founder/Admin-only and never returns secrets", async () => {
  ...
  assert.match(source, /PHONE_WABA_ACTUAL_ID/);
  assert.match(source, /WABA_ACCESS_STATUS/);
  assert.match(source, /SUBSCRIBED_APP_FOUND/);
  assert.match(source, /NOT_FOUND/);
  assert.match(source, /FORBIDDEN/);
  assert.match(source, /errorSubcode/);
...
test("AI health probe is Founder/Admin-only, uses generateObject and never sends WhatsApp", async () => {
  ...
  assert.match(source, /createOpenAI\(\{ apiKey: process\.env\.OPENAI_API_KEY \}\)/);
  assert.match(source, /ORBIT_WHATSAPP_AI_PROVIDER/);
  assert.match(source, /OPENAI_API_KEY_MISSING/);
  assert.match(source, /generateObject\(\{/);
  assert.doesNotMatch(source, /providerOptions:\s*\{\s*gateway/);

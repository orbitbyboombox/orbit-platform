...
-- Global search keeps text/name/email behavior and adds an exact canonical
-- branch for explicit international phone queries. Legacy fallback is only
-- used when the stored row has no canonical value; no country is inferred.
...
as $$
with input as (
  select trim(coalesce(p_query, '')) raw_term,
    public.orbit_search_normalize(p_query) term,
    case when left(trim(coalesce(p_query, '')), 1) = '+' then public.normalize_phone_e164(p_query) end phone_term,
  ...
  select 'CUSTOMER'::text entity_type, c.id entity_id,
    ...
    concat_ws(' · ', nullif(c.email, ''), nullif(coalesce(c.phone_e164, c.phone), ''), nullif(c.rut, '')) subtitle,
    case when i.phone_term is not null then 0
         when public.orbit_search_normalize(c.full_name) = i.term then 0
  ...
  where c.deleted_at is null and (
    (i.phone_term is not null and (c.phone_e164 = i.phone_term or (c.phone_e164 is null and public.normalize_phone_e164(c.phone) = i.phone_term)))
    or (i.phone_term is null and length(i.term) >= 2 and public.orbit_search_normalize(concat_ws(' ', c.full_name, c.email, c.phone, c.rut)) like '%' || i.term || '%')
  )

  union all

  select 'COMPANY'::text, c.id, c.company,
    concat_ws(' · ', nullif(c.full_name, ''), nullif(c.email, ''), nullif(coalesce(c.phone_e164, c.phone), '')),
    case when public.orbit_search_normalize(c.company) = i.term then 0 else 2 end
  from public.customers c cross join manager_access i
  where length(i.term) >= 2 and i.phone_term is null and c.deleted_at is null
  ...
  select 'EVENT'::text, p.id,
    ...
    concat_ws(' · ', nullif(c.full_name, ''), nullif(c.company, ''), to_char(p.event_date, 'DD-MM-YYYY'), nullif(string_agg(distinct ps.service_code, ' + ' order by ps.service_code), '')),
  ...
  from public.projects p join public.customers c on c.id = p.customer_id
    left join public.project_services ps on ps.project_id = p.id cross join input i
  where length(i.term) >= 2 and i.phone_term is null and p.deleted_at is null and c.deleted_at is null
    and upper(coalesce(p.status, '')) not in ('CANCELLED','CANCELED','ARCHIVED')
    and ((i.is_internal and i.role <> 'STAFF') or (i.role = 'STAFF' and exists (
  ...
  from public.quotations q left join public.customers c on c.id = q.customer_id cross join manager_access i
  where length(i.term) >= 2 and i.phone_term is null and q.deleted_at is null
...
order by case ranked.entity_type when 'CUSTOMER' then 1 when 'COMPANY' then 2 when 'EVENT' then 3 else 4 end, ranked.relevance, ranked.title;
$$;

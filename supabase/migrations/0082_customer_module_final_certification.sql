...
begin
  ...
  if p_changes ? 'boothQuantity' then
  ...
  select id into target_quotation_id from quotations where project_id=p_project_id and deleted_at is null order by created_at desc limit 1;
  if target_quotation_id is not null then
    ...
    where project_id=p_project_id and status not in ('SIGNED','COMMERCIAL_DOCUMENT');
  end if;

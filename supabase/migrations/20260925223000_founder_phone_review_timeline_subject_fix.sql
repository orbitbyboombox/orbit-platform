...
begin
  ...
  if actor_role not in ('CEO', 'ADMINISTRATOR') then raise exception 'PHONE_REVIEW_FORBIDDEN'; end if;
  ...
  for row_customer in select * from public.customers c where c.id = any(p_customer_ids) order by c.id for update loop
    ...
    select coalesce(
      (select p.orbit_event_id from public.projects p where p.customer_id = row_customer.id and p.deleted_at is null and nullif(trim(p.orbit_event_id), '') is not null order by p.event_date desc nulls last, p.created_at desc limit 1),
      'CRM-CUSTOMER-' || row_customer.id::text
    ) into audit_orbit_event_id;

    insert into public.timeline_events(
      customer_id, orbit_event_id, event_type, title, description, actor_id, actor_label,
      source, action, entity_type, entity_id, human_message, correlation_id, created_by
    ) values (
      row_customer.id, audit_orbit_event_id, 'CUSTOMER_PHONE_REVIEWED', 'Revisión de teléfono',
      jsonb_build_object('decision', p_decision, 'oldPhone', row_customer.phone, 'oldPhoneE164', row_customer.phone_e164,
        'newPhoneE164', case when p_decision = 'CANONICALIZED' then canonical else row_customer.phone_e164 end, 'reviewedAt', now())::text,
      actor, case when actor_role = 'CEO' then 'Founder' else 'Administrador' end, 'Administrator',
      'CUSTOMER_PHONE_REVIEW', 'Customer', row_customer.id, 'Revisión de teléfono registrada por Administración.',
      'customer-phone-review:' || row_customer.id::text || ':' || extract(epoch from clock_timestamp())::bigint, actor
    );

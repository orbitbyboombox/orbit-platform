-- Canonical Staff advances may carry their receipt and optional boleta.
alter table public.event_staff_settlement_movements
  add column if not exists receipt_document_id uuid references public.staff_onboarding_documents(id),
  add column if not exists boleta_document_id uuid references public.staff_onboarding_documents(id);
create index if not exists event_staff_settlement_movements_receipt_idx on public.event_staff_settlement_movements(receipt_document_id) where receipt_document_id is not null;
...
begin
  ...
  values(project_row.id,project_row.customer_id,'STAFF_ADVANCE_REGISTERED','Adelanto Staff registrado','Adelanto operativo registrado en el ledger Staff.', 'ADVANCE',coalesce(nullif(trim(p_notes),''),'Adelanto registrado por Founder.'),'STAFF-ADVANCE-'||p_idempotency_key,actor);
...
revoke all on function public.register_staff_advance_with_documents(uuid,numeric,date,text,text,text,text,text,text,text,text,text,text,text) from public,anon;
grant execute on function public.register_staff_advance_with_documents(uuid,numeric,date,text,text,text,text,text,text,text,text,text,text,text) to authenticated,service_role;

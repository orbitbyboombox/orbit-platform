...
create table if not exists public.event_deletion_jobs (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null unique references public.projects(id),
  customer_id uuid references public.customers(id),
  actor_id uuid references auth.users(id),
  reason text not null,
  delete_orphan_customer boolean not null default false,
  status text not null default 'REQUESTED' check (status in ('REQUESTED','REMOVED_FROM_OPERATION','EXTERNAL_CLEANUP','COMPLETED','FAILED')),
...
create index if not exists event_deletion_jobs_retry_idx
  on public.event_deletion_jobs(status, next_retry_at);

alter table public.event_deletion_jobs enable row level security;
drop policy if exists event_deletion_jobs_founder_read on public.event_deletion_jobs;
create policy event_deletion_jobs_founder_read on public.event_deletion_jobs
  for select to authenticated using (public.current_orbit_role() = 'CEO');
...
begin
  ...
  -- Remove operational exposure first. Append-only timeline rows remain as a
  -- hidden audit trail and never block the Founder action.
  ...
  delete from public.reservation_commercial_negotiations where project_id=p_project_id;
  ...
  delete from public.quotations where project_id=p_project_id;
  ...
  if p_delete_orphan_customer and not customer_has_other_events then
    update public.customers set deleted_at=now(), deleted_by=actor, updated_by=actor, updated_at=now(), approval_reason='SOURCE_EVENT_DELETED' where id=project_row.customer_id and deleted_at is null;
  end if;
...
revoke all on function public.purge_event_controlled(uuid,text,text,boolean) from public,anon;
grant execute on function public.purge_event_controlled(uuid,text,text,boolean) to authenticated;
grant execute on function public.purge_event_controlled(uuid,text,text,boolean) to service_role;

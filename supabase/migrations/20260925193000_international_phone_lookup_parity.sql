...
create index if not exists customers_phone_e164_lookup_idx
...
declare
  actor uuid:=auth.uid();
  d_customer jsonb:=p_draft->'client';
  ...
  crm_id uuid;
  event_id text;
  new_customer boolean:=false;
  service text;
  stage text:='Customer Lookup';
  error_state text;
  error_message text;
  error_detail text;
  normalized_rut text:=regexp_replace(upper(coalesce(d_customer->>'rut','')),'[^0-9K]','','g');
  normalized_phone text:=public.normalize_phone_e164(d_customer->>'phone');
begin
  ...
  if requested_customer_id is not null then
    ...
    select c.id into c_id from customers c
    where c.deleted_at is null and (
      ...
      or (normalized_phone is not null and c.phone_e164=normalized_phone)
      or (normalized_phone is null and nullif(regexp_replace(d_customer->>'phone','[^0-9]','','g'),'') is not null and regexp_replace(c.phone,'[^0-9]','','g')=regexp_replace(d_customer->>'phone','[^0-9]','','g'))
  ...
  if c_id is null then
    ...
    insert into customers(id,full_name,email,secondary_email,phone,phone_e164,phone_e164_status,company,rut,address,city,metadata,created_by,updated_by)
    values(c_id,d_customer->>'name',nullif(lower(trim(d_customer->>'email')),''),nullif(lower(trim(d_customer->>'secondaryEmail')),''),nullif(d_customer->>'phone',''),normalized_phone,case when nullif(trim(d_customer->>'phone'),'') is null then 'EMPTY' when normalized_phone is not null then 'VALID_E164' when regexp_replace(trim(d_customer->>'phone'),'[[:space:]()\-]','','g') ~ '^[0-9]+$' then 'AMBIGUOUS' else 'INVALID' end,nullif(d_customer->>'company',''),nullif(d_customer->>'rut',''),nullif(d_customer->>'address',''),d_event->>'city',jsonb_build_object('leadSource',p_draft->>'origin'),actor,actor);
  end if;

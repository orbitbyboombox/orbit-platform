...
create or replace function public.confirm_reservation_operational_pipeline(p_project_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
declare actor uuid:=auth.uid();p public.projects%rowtype;q_id uuid;estimate public.estimated_cost_sheets%rowtype;truth public.financial_event_records%rowtype;profit_id uuid;
begin
  if actor is null or not public.is_internal_user()then raise exception'Acceso interno requerido.';end if;
  ...
  perform public.sync_estimated_cost_sheet(p.id);
  perform public.sync_financial_event(p.id);
  perform public.sync_event_operation_cost(p.id);
  profit_id:=public.sync_event_profitability(p.id);

  select * into estimate from public.estimated_cost_sheets where project_id=p.id;
  select * into truth from public.financial_event_records where project_id=p.id;
  ...
  if not exists(select 1 from public.timeline_events where correlation_id='confirmed-operation-pipeline:'||p.id)then
    insert into public.timeline_events(customer_id,project_id,orbit_event_id,event_type,title,description,actor_id,actor_label,source,action,entity_type,entity_id,human_message,correlation_id,new_state,created_by)
    values(p.customer_id,p.id,p.orbit_event_id,'OPERATION_PIPELINE_CONFIRMED','Pipeline operacional confirmado','Costos, rentabilidad, Dashboard y Business Intelligence sincronizados.',actor,'ORBIT','System','OPERATION_PIPELINE_CONFIRMED','Project',p.id,'La reserva utiliza el pipeline operacional unificado.','confirmed-operation-pipeline:'||p.id,'CONFIRMED',actor);
  end if;
...
revoke all on function public.confirm_reservation_operational_pipeline(uuid)from public,anon;
grant execute on function public.confirm_reservation_operational_pipeline(uuid)to authenticated;

...
-- Keep expense materialization, reimbursement projection, monthly payroll and
-- the Founder audit trail in one canonical transaction.
...
begin
  ...
  select customer_id,event_date,orbit_event_id
  into project_customer,project_event_date,event_code
  ...
  if p_action='REJECT' then
    ...
    insert into public.timeline_events(
      ...
      project_customer,item.project_id,item.staff_id,event_code,'STAFF_EXPENSE_REJECTED',
      'Gasto Staff rechazado',concat(item.category,' · $',item.amount),'Administrador',
      'Operations','STAFF_EXPENSE_REJECTED','StaffExpenseSubmission',item.id,
      'Gasto Staff rechazado sin impacto financiero.','staff-expense-rejected:'||item.id,trim(p_reason)
  ...
  insert into public.expenses(
    ...
    jsonb_build_object(
      'associationType','EVENT','associationId',item.project_id,'assignmentIds',to_jsonb(assignment_ids),
      'responsibleStaffId',item.staff_id,'eventStaffSettlementId',case when item.reimbursement then settlement_id else null end,
      'staffExpenseSubmissionId',item.id,'description',item.description,'paymentMethod',item.payment_method,
  ...
  update public.documents set expense_id=created_expense_id
  ...
  perform public.ensure_staff_monthly_account(item.staff_id,project_event_date);

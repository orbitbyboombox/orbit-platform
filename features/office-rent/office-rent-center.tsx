...
import { formatClp, monthLabel, receiptLabel, type OfficeLeaseDataset, type OfficeLeaseDocument, type OfficeLeaseMonth, type OfficeLeaseStatus } from "./model";
...
const date = (value: string | null) => value
  ? new Date(`${value}T12:00:00Z`).toLocaleDateString("es-CL", { day: "2-digit", month: "short", year: "numeric", timeZone: "UTC" })
  : "Sin fecha";
...
export function OfficeRentCenter({ data }: { data: OfficeLeaseDataset }) {
  ...
  return <main className="space-y-6 pb-12">
    ...
    <section className="grid grid-cols-2 gap-3 xl:grid-cols-4">
      ...
      <MetricCard detail={`${data.metrics.paidMonths} meses pagados · ${data.metrics.pendingMonths} pendientes`} icon={<ReceiptText className="size-4 text-brand"/>} label="Recibido este año" value={formatClp(data.metrics.yearReceived)}/>
      <MetricCard detail={data.metrics.nextDueDate ? `Vence ${date(data.metrics.nextDueDate)}` : "Sin obligación próxima"} icon={<CalendarClock className="size-4 text-brand"/>} label="Próximo vencimiento" value={data.metrics.nextDueDate ? date(data.metrics.nextDueDate) : "Al día"}/>
    </section>

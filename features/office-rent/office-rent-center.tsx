...
function OfficeRentHydrated({ data }: { data: OfficeLeaseDataset }) {
  ...
  return <main className="space-y-6 pb-12">
    ...
    <section className="rounded-3xl border bg-card p-4 sm:p-6">
      ...
      <div className="mt-5 space-y-3 md:hidden">{data.months.map((month) => <article className="rounded-2xl border bg-background/35 p-4" key={month.id}><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{monthLabel(month.period)}</p><p className="mt-1 text-sm text-muted">{formatClp(month.amountDue)} · vence {formatOfficeDate(month.dueDate)}</p></div><span className={cn("rounded-full border px-2.5 py-1 text-xs font-semibold", statusMeta[month.status].className)}>{statusMeta[month.status].label}</span></div>{month.payments.map((payment) => { const receipt = documentsByPayment(payment.id, "INCOME_RECEIPT"); const proof = documentsByPayment(payment.id, "PAYMENT_PROOF"); return <div className="mt-3 rounded-xl border p-3 text-xs" key={payment.id}><p>{formatOfficeDate(payment.paidOn)} · {formatClp(payment.amount)} · {payment.paymentMethod}</p><div className="mt-2 flex flex-wrap gap-1">{receipt ? <DocumentLinks document={receipt}/> : null}{proof ? <DocumentLinks document={proof}/> : null}</div></div>; })}{month.outstandingAmount > 0 ? <Button className="mt-3 w-full" onClick={() => openPayment(month)} size="sm">Registrar {formatClp(month.outstandingAmount)}</Button> : null}</article>)}</div>
    </section>

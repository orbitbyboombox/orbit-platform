...
export async function createQuotationPdf(input: QuotationPdfInput): Promise<Uint8Array> {
  ...
  page.drawText("ORBIT by BOOMBOX", { x:42,y:795,size:20,font:bold,color:orange });
  ...
  for (const item of input.items.slice(0,12)) { page.drawText(`${safe(item.label)} × ${item.quantity}`,{x:42,y,size:10,font,color:dark}); page.drawText(money(item.total),{x:455,y,size:10,font:bold,color:dark}); y-=24; }
  y-=12; page.drawLine({start:{x:330,y},end:{x:553,y},thickness:0.8,color:rgb(0.82,0.83,0.85)}); y-=24;
  for (const [label,value] of [["Subtotal",input.subtotal],["Transporte",input.transport],["IVA",input.taxes]] as const) { page.drawText(label,{x:350,y,size:9,font,color:muted}); page.drawText(money(value),{x:455,y,size:9,font,color:dark}); y-=21; }
  y=Math.max(82,y); page.drawText("TOTAL",{x:350,y,size:11,font:bold,color:dark}); page.drawText(money(input.total),{x:455,y,size:12,font:bold,color:orange});
  page.drawText("ORBIT v1.0 · Developed by BOOMBOX · Powered by NOVA CORE",{x:42,y:48,size:8,font,color:muted});
  return pdf.save();
}

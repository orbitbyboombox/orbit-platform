...
import { CalendarDays, ChevronDown, ExternalLink, FileText, FolderSync, Pencil, ShieldCheck, X } from "lucide-react";
...
export function CustomerEventOperations({ event, operations, onEditEvent }: { event: CrmEventSummary; operations?: CrmCustomerEventOperations; onEditEvent: () => void }) {
  ...
  return <div className="mt-4 space-y-5 border-t pt-5">
    ...
    <section className="scroll-mt-24 rounded-2xl border bg-card p-5 sm:p-6" id={`profitability-${event.projectId}`}>
      ...
      {operations.profitability ? <><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><ProfitMetric label="Ingresos" value={money(operations.profitability.revenue)}/><ProfitMetric label="Costo de personal" value={money(operations.profitability.personnelCost)}/><ProfitMetric label="Costo operacional" value={money(operations.profitability.operationalCost)}/><button aria-expanded={costDetailOpen} className="rounded-xl border bg-background/30 p-4 text-left transition hover:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand" onClick={() => setCostDetailOpen((open) => !open)} type="button"><span className="flex items-center justify-between gap-3"><span><span className="block text-xs text-muted">Costo total</span><strong className="mt-2 block text-lg">{money(operations.profitability.totalCost)}</strong></span><ChevronDown className={`size-5 text-brand transition ${costDetailOpen ? "rotate-180" : ""}`}/></span><span className="mt-2 block text-xs text-brand">Ver costos considerados</span></button><ProfitMetric label="Profit" value={money(operations.profitability.profit)}/><ProfitMetric label="Margen" value={`${operations.profitability.margin.toFixed(1)}%`}/></div>{costDetailOpen && <CostDetail profitability={operations.profitability}/>}<p className="mt-4 text-xs text-muted">{profitabilityLabel(operations.profitability.classification)} · Calculado {new Date(operations.profitability.calculatedAt).toLocaleString("es-CL")}</p></> : <p className="mt-3 text-sm text-muted">Aún no existe un cálculo de rentabilidad para este Evento.</p>}
    </section>
...
function CostDetail({ profitability }: { profitability: NonNullable<CrmCustomerEventOperations["profitability"]> }) {
  const listedTotal = profitability.costBreakdown.reduce((sum, item) => sum + item.amount, 0);
  const reconciliation = profitability.totalCost - listedTotal;
  const groups = [
  ...
  return <div className="mt-4 rounded-xl border border-brand/25 bg-brand/5 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="font-semibold">Detalle del costo usado para calcular el margen</h4><p className="mt-1 text-xs text-muted">Valores oficiales del Business Engine para este Evento.</p></div><strong className="text-brand">{money(profitability.totalCost)}</strong></div><div className="mt-4 grid gap-4 lg:grid-cols-2">{groups.map((group) => <div className="rounded-xl border bg-background/40 p-4" key={group.key}><div className="flex justify-between gap-3 border-b pb-2 text-sm font-semibold"><span>{group.title}</span><span>{money(group.total)}</span></div><dl className="mt-2 space-y-2">{profitability.costBreakdown.filter((item) => item.group === group.key).map((item) => <div className="flex justify-between gap-3 text-sm" key={item.key}><dt className="text-muted">{item.label}</dt><dd>{money(item.amount)}</dd></div>)}</dl></div>)}</div>{Math.abs(reconciliation) >= 1 && <div className="mt-3 flex justify-between gap-3 rounded-lg border border-warning/30 bg-warning/5 px-3 py-2 text-sm"><span>Otros costos considerados por el motor</span><strong>{money(reconciliation)}</strong></div>}<div className="mt-4 flex justify-between gap-3 border-t pt-3 font-semibold"><span>Costo total aplicado</span><span>{money(profitability.totalCost)}</span></div></div>;
}

...
export async function loadCrmCustomerProfile(client:SupabaseClient,customerId:string):Promise<CrmCustomerProfile|null>{
  const[{data:customer,error},{data:events,error:eventError},{data:payments,error:paymentError},{data:invoices,error:invoiceError},{data:timeline,error:timelineError}]=await Promise.all([
  ...
  ]);const firstError=error??eventError??paymentError??invoiceError??timelineError;if(firstError)throw firstError;if(!customer)return null;

...
type GlobalWorkspaceTarget={element:HTMLElement;host:HTMLElement};
...
export function GlobalLayoutEngine(){
  ...
  if(!context||!moduleKey)return null;const labels=context.preferences.moduleWorkspaces[moduleKey].sectionLabels??{};return <>{Object.entries(targets).map(([key,{element,host}])=>createPortal(<div className="absolute right-0 top-0 z-30" data-workspace-control="true"><WorkspaceSectionMenu label={labels[key]??element.dataset.workspaceLabel??key} moduleKey={moduleKey} sectionKey={key}/></div>,host,`global-layout-${moduleKey}-${key}`))}</>;
}
...
function workspaceMenuHost(element:HTMLElement){const explicit=element.querySelector<HTMLElement>(":scope > [data-workspace-header]");if(explicit)return explicit;const header=element.querySelector<HTMLElement>(":scope > header");if(header)return header;const first=element.firstElementChild instanceof HTMLElement?element.firstElementChild:null;if(first?.querySelector("h1,h2,h3,summary")||first?.matches("h1,h2,h3,summary"))return first;return element}

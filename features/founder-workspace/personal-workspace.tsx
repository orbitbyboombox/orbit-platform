...
import { type FounderWorkspacePreferences, type ModuleWorkspaceKey } from "./catalog";
...
export function GlobalLayoutEngine(){
  const pathname=usePathname();const context=useContext(WorkspaceContext);const[targets,setTargets]=useState<Record<string,GlobalWorkspaceTarget>>({});const moduleKey=ROUTE_MODULES.find(([pattern])=>pattern.test(pathname))?.[1];
  useEffect(()=>{if(!context||!moduleKey)return;let timer:ReturnType<typeof setTimeout>;const discover=()=>{const root=document.getElementById("platform-workspace-content");if(!root)return;const candidates=[...root.querySelectorAll<HTMLElement>(BLOCK_SELECTOR)].filter(element=>!element.closest('[data-workspace-ignore]')&&!element.dataset.workspaceControl&&element.offsetParent!==null);const used=new Map<string,number>();const found:Record<string,GlobalWorkspaceTarget>={};const labels:Record<string,string>={};for(const element of candidates){const label=workspaceLabel(element);const base=element.dataset.workspaceKey||element.id||slug(label)||element.tagName.toLowerCase();const count=used.get(base)??0;used.set(base,count+1);const key=count?`${base}-${count+1}`:base;const header=workspaceMenuHost(element);const title=workspaceHeaderTitle(element,header);element.dataset.workspaceKey=key;element.dataset.workspaceLabel=label;element.style.position="relative";prepareWorkspaceHeader(header);found[key]={element,menuTop:workspaceMenuTop(element,title)};labels[key]=label}setTargets(found);const config=context.preferences.moduleWorkspaces[moduleKey];const keys=Object.keys(found);const missing=keys.filter(key=>!config.sectionOrder.includes(key));const changedLabels=keys.some(key=>config.sectionLabels?.[key]!==labels[key]);if(missing.length||changedLabels)context.update({...context.preferences,moduleWorkspaces:{...context.preferences.moduleWorkspaces,[moduleKey]:{...config,sectionOrder:[...config.sectionOrder,...missing],hiddenSections:config.hiddenSections,sectionLabels:{...config.sectionLabels,...labels}}}})};discover();const observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(discover,80)});const root=document.getElementById("platform-workspace-content");if(root)observer.observe(root,{childList:true,subtree:true});window.addEventListener("resize",discover);return()=>{clearTimeout(timer);observer.disconnect();window.removeEventListener("resize",discover)}},[context,moduleKey,pathname]);
  ...
  if(!context||!moduleKey)return null;const labels=context.preferences.moduleWorkspaces[moduleKey].sectionLabels??{};return <>{Object.entries(targets).map(([key,{element,menuTop}])=>createPortal(<div className="absolute right-4 z-30" data-workspace-control="true" style={{top:menuTop}}><WorkspaceSectionMenu label={labels[key]??element.dataset.workspaceLabel??key} moduleKey={moduleKey} sectionKey={key}/></div>,element,`global-layout-${moduleKey}-${key}`))}</>;
}
...
function workspaceMenuHost(element:HTMLElement){const explicit=element.querySelector<HTMLElement>(":scope > [data-workspace-header]");if(explicit)return explicit;const header=element.querySelector<HTMLElement>(":scope > header");if(header)return header;const title=element.querySelector<HTMLElement>("h1,h2,h3,summary");if(title?.parentElement&&element.contains(title.parentElement))return title.parentElement;return element}
function workspaceHeaderTitle(element:HTMLElement,host:HTMLElement){return host.matches("h1,h2,h3,summary")?host:host.querySelector<HTMLElement>("h1,h2,h3,summary")??element.querySelector<HTMLElement>("h1,h2,h3,summary")}
function prepareWorkspaceHeader(host:HTMLElement){if(!host.dataset.workspaceOriginalPaddingRight)host.dataset.workspaceOriginalPaddingRight=getComputedStyle(host).paddingRight;host.style.position="relative";host.style.paddingRight=`calc(${host.dataset.workspaceOriginalPaddingRight} + 3.5rem)`}
function workspaceMenuTop(host:HTMLElement,title:HTMLElement|null){if(!title)return 0;const hostRect=host.getBoundingClientRect();const titleRect=title.getBoundingClientRect();return Math.max(0,titleRect.top-hostRect.top+(titleRect.height-44)/2)}

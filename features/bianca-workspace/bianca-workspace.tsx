...
export function BiancaWorkspace({ projection, status, whatsappConnected, deliveryEnabled, deliveryLabels }: { projection: CommunicationHubProjection; status: BiancaOperationalStatus; whatsappConnected: boolean; deliveryEnabled: boolean; deliveryLabels: { engine: string; messaging: string; ai: string } }) {
  ...
  return <main className="mx-auto w-full max-w-[1480px] space-y-6 px-4 pb-10 sm:px-6 lg:px-8">
    <header className="rounded-2xl border bg-card p-5 sm:p-7">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-muted"><span className="inline-flex items-center gap-1.5"><ShieldCheck className="size-4" />Delivery automático: {deliveryEnabled ? "ON" : "OFF"}</span><span>Requieren tu ayuda: <b className="text-foreground">{attention}</b></span><Link className="font-semibold text-brand hover:underline" href="/settings/bianca-lab">Abrir BIANCA Lab <ExternalLink className="ml-1 inline size-3" /></Link></div>
    </header>
    ...
    <WhatsAppInbox conversations={projection.conversations} deliveryEnabled={deliveryEnabled} events={projection.events} workspaceMode />
  </main>;
}

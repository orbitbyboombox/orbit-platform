  ...
  return <section className="mb-5 space-y-3 rounded-[24px] border border-white/10 bg-[#111214] p-3 text-white shadow-[0_20px_70px_rgba(0,0,0,.2)] sm:p-4">
    ...
    <div className="rounded-2xl border border-brand/30 bg-[#191a1d] p-3 shadow-[0_0_35px_rgba(247,137,0,.08)] sm:p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
        <div className="grid size-20 shrink-0 place-items-center rounded-xl bg-brand text-center text-black"><span className="text-[10px] uppercase tracking-[.16em]">{new Date(`${date}T12:00:00Z`).toLocaleDateString("es-CL", { weekday: "short" })}</span><strong className="text-3xl leading-none">{date.slice(8, 10)}</strong><span className="text-[10px]">{date.slice(5, 7)}</span></div>
        <div className="min-w-0 flex-1"><p className="text-xs text-white/55">{eventType}</p><h1 className="mt-0.5 text-2xl font-semibold leading-tight sm:text-3xl">{customer}</h1><p className="mt-2 text-sm font-semibold text-brand">{serviceLabel}</p><p className="mt-1 text-base font-semibold tracking-tight">{timeRange}</p><p className="mt-0.5 text-xs text-white/65">Citación Staff: {staffCallTime ? `${staffCallTime} hrs` : "Por confirmar"}</p><span className="mt-2 inline-flex rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-300">{status}</span></div>
      </div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2"><Info label="Lugar" value={display(venue)}/><Info label="Comuna" value={display(municipality)}/></div>
      <div className="mt-3"><p className="text-[11px] uppercase tracking-[.16em] text-white/45">Extras</p><div className="mt-1.5 flex flex-wrap gap-1.5">{normalizedExtras.length ? normalizedExtras.map((item) => <span className="rounded-full border border-brand/30 bg-brand/10 px-2.5 py-1 text-[11px] font-semibold text-brand" key={item}>{item}</span>) : <span className="text-xs text-white/60">SIN EXTRAS</span>}</div></div>
      <div className="mt-3 rounded-xl border border-white/10 bg-white/[.03] p-3"><p className="text-[11px] uppercase tracking-[.16em] text-white/45">Encargado</p><p className="mt-1 text-sm font-semibold">{contactName}</p><p className="text-xs text-white/65">{contactPhone}</p></div>
      <div className="mt-3 flex flex-wrap gap-2"><a className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-brand px-3.5 text-xs font-semibold text-black" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${venue}, ${municipality}`)}`} rel="noreferrer" target="_blank"><MapPin className="size-3.5"/>VER UBICACIÓN</a>{hasPhone ? <a className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-brand/40 px-3.5 text-xs font-semibold text-brand" href={phoneHref(operationalContactPhone)}><Phone className="size-3.5"/>CONTACTAR</a> : null}</div>
    </div>
    <div className="grid gap-3 lg:grid-cols-2">
      <div className="rounded-2xl border border-white/10 bg-[#191a1d] p-3"><div className="flex items-center justify-between"><div><p className="text-[11px] uppercase tracking-[.16em] text-brand">Operación</p><h2 className="mt-0.5 text-sm font-semibold">OPERADORES DEL EVENTO</h2></div><span className="text-[11px] text-white/45">Citación {staffCallTime ? `${staffCallTime} hrs` : "por confirmar"}</span></div><div className="mt-2 divide-y divide-white/10">{operators.map((operator) => <div className="flex items-center justify-between gap-3 py-2" key={operator.role}><div><p className="text-[10px] uppercase tracking-[.14em] text-white/45">{roleLabel(operator.role)}</p><p className="text-xs font-semibold">{operator.name || "Sin asignar"}</p></div><span className="text-right text-[11px] text-white/55">{operator.callTime ? `${operator.callTime} hrs` : "Sin citación"}</span></div>)}</div><div className="mt-2 grid grid-cols-4 gap-1 text-[10px]">{["CHECK-OUT", "EVENTO", "PAPEL", "CHECK-IN"].map((item) => <span className="rounded-full border border-white/10 px-1 py-1 text-center text-white/65" key={item}>{item}</span>)}</div></div>
      <div className="rounded-2xl border border-brand/25 bg-[#241812] p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-[11px] uppercase tracking-[.16em] text-brand">Papel / impresión</p><h2 className="mt-0.5 text-sm font-semibold">Resumen del evento</h2></div><span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] text-white/55">{paper?.status ?? "PENDIENTE"}</span></div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><PaperMetric label="Papel ida" value={paper?.opening === null || paper?.opening === undefined ? "PENDIENTE" : String(paper.opening)}/><PaperMetric label="Papel final" value={paper?.final === null || paper?.final === undefined ? "PENDIENTE" : String(paper.final)}/><PaperMetric label="Tipo de papel" value={paper?.variant === "NORMAL_4X6" ? "4x6 NORMAL" : paper?.variant === "PRECUT_4X6" ? "4x6 PREPICADO" : "PENDIENTE"}/><PaperMetric label="Impresiones" value={paper?.usage === null || paper?.usage === undefined ? "PENDIENTE" : String(paper.usage)}/></div><div className="mt-2"><p className="text-[10px] uppercase tracking-[.14em] text-white/45">Tipo de papel</p><div className="mt-1 flex flex-wrap gap-1.5">{([['NORMAL_4X6','4x6 NORMAL'],['PRECUT_4X6','4x6 PREPICADO']] as const).map(([variant,label]) => <button className={`min-h-8 rounded-lg border px-2.5 text-[11px] font-semibold transition ${paper?.variant === variant ? "border-brand bg-brand/15 text-brand" : "border-white/10 text-white/70 hover:border-brand/50"}`} disabled={paperPending || !paper || paper.status === "CONFIRMED" || paper.status === "OVERRIDDEN"} key={variant} onClick={() => startPaperTransition(async () => { try { await updateEventPaperVariantAction({ projectId, variant }); setPaperMessage("Tipo de papel guardado."); } catch (error) { setPaperMessage(error instanceof Error ? error.message : "No fue posible guardar el tipo de papel."); } })} type="button">{label}</button>)}</div>{paperMessage ? <p className="mt-1 text-[10px] text-white/60">{paperMessage}</p> : null}</div></div>
    </div>
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/10 bg-[#191a1d] px-3 py-2.5"><div className="min-w-0"><p className="text-[10px] uppercase tracking-[.16em] text-brand">Caja Negra</p><p className="truncate text-xs text-white/70">{["Caja Negra", "Impresora", "Cámara", "Pantalla", ...equipment].filter((item, index, all) => item && all.indexOf(item) === index).join(" · ") || "Equipamiento pendiente"}</p></div><Link className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded-lg border border-brand/40 px-2.5 text-[11px] font-semibold text-brand" href={`/projects/${projectId}?experience=operations`}>VER EQUIPAMIENTO <ChevronRight className="size-3.5"/></Link></div>

...
export function EventUiReplica({ projectId, customer, date, service, serviceDuration, serviceStartTime, serviceEndTime, staffCallTime, venue, municipality, status, eventType, extras, operationalContactName, operationalContactPhone, operators, paper, invoice, equipment }: Props) {
  ...
  return <section className="mb-5 space-y-3 rounded-[24px] border border-white/10 bg-[#111214] p-3 text-white shadow-[0_20px_70px_rgba(0,0,0,.2)] sm:p-4">
    ...
    <div className="rounded-2xl border border-brand/30 bg-[#191a1d] p-3 shadow-[0_0_35px_rgba(247,137,0,.08)] sm:p-4">
      ...
      <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(21rem,0.9fr)]">
        <div className="rounded-xl border border-white/10 bg-white/[.03] p-3">
          <div className="space-y-2"><Info label="Lugar" value={display(venue)}/><Info label="Comuna" value={display(municipality)}/></div>
          <div className="mt-2"><p className="text-[11px] uppercase tracking-[.16em] text-white/45">Extras</p><div className="mt-1.5 flex flex-wrap gap-1.5">{normalizedExtras.length ? normalizedExtras.map((item) => <span className="rounded-full border border-brand/30 bg-brand/10 px-2.5 py-1 text-[11px] font-semibold text-brand" key={item}>{item}</span>) : <span className="text-xs text-white/60">SIN EXTRAS</span>}</div></div>
          <div className="mt-2 rounded-lg border border-white/10 px-2.5 py-2"><p className="text-[11px] uppercase tracking-[.16em] text-white/45">Encargado del evento</p><p className="mt-1 text-sm font-semibold">{contactName}</p><p className="text-xs text-white/65">{contactPhone}</p></div>
          <div className="mt-2 flex flex-wrap gap-2"><a className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-brand px-3.5 text-xs font-semibold text-black" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${venue}, ${municipality}`)}`} rel="noreferrer" target="_blank"><MapPin className="size-3.5"/>VER UBICACIÓN</a>{hasPhone ? <a className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-brand/40 px-3.5 text-xs font-semibold text-brand" href={phoneHref(operationalContactPhone)}><Phone className="size-3.5"/>CONTACTAR</a> : null}</div>
        </div>

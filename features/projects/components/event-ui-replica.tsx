...
export function EventUiReplica({ projectId, customer, date, service, serviceDuration, serviceStartTime, serviceEndTime, staffCallTime, venue, municipality, status, eventType, extras, operationalContactName, operationalContactPhone, operators, paper, invoice, equipment }: Props) {
  ...
  return <section className="mb-7 space-y-4 rounded-[28px] border border-white/10 bg-[#111214] p-4 text-white shadow-[0_20px_70px_rgba(0,0,0,.2)] sm:p-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-brand/25 bg-[#241812] p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs uppercase tracking-[.16em] text-brand">Papel / impresión</p><h2 className="mt-1 font-semibold">Resumen del evento</h2></div><span className="rounded-full border border-white/10 px-2 py-1 text-[11px] text-white/55">{paper?.status ?? "PENDIENTE"}</span></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><PaperMetric label="Papel ida" value={paper?.opening === null || paper?.opening === undefined ? "PENDIENTE" : String(paper.opening)}/><PaperMetric label="Papel final evento" value={paper?.final === null || paper?.final === undefined ? "PENDIENTE" : String(paper.final)}/><PaperMetric label="Tipo de papel" value={paper?.format ?? "PENDIENTE"}/><PaperMetric label="Impresiones utilizadas" value={paper?.usage === null || paper?.usage === undefined ? "PENDIENTE" : String(paper.usage)}/></div><p className="mt-3 text-xs text-white/50">Fórmula C.5: apertura + recargas − final.</p></div>
    </div>

...
const projectType = (value: string): ProjectType =>
  [
    "Wedding",
    "Corporate",
    "Birthday",
...
const projectStatus = (value: string): ProjectStatus =>
  ["Active", "Upcoming", "Completed", "Archived"].includes(value)
    ? (value as ProjectStatus)
    : "Upcoming";
const projectHealth = (value: string): ProjectHealth =>
  ["Healthy", "Attention", "Risk", "Critical"].includes(value)
    ? (value as ProjectHealth)
    : "Healthy";
const commercialStage = (value: unknown): ProjectCommercialStage =>
  [
    "New",
    "Contacted",
    "Quoting",
    ...
    ? (value as ProjectCommercialStage)
    : "New";
const origin = (value: unknown): ProjectOrigin | undefined =>
  [
    "WhatsApp",
    "Instagram",
    "Google",
...
export class SupabaseCustomerRepository implements CustomerRepository {
  ...
  constructor(private readonly client: SupabaseClient) {
    this.timeline = new SupabaseTimelineRepository(client);
  }
  ...
  async findAll(): Promise<Project[]> {
    const [
      ...
      this.client
        ...
        .select("customer_id,project_id,human_message,occurred_at")
    ...
    const customerMap = new Map(
      (customers as CustomerRow[]).map((row) => [row.id, row]),
    );
    ...
    const memoryMap = new Map(
      (memories as MemoryRow[]).map((row) => [row.customer_id, row.context]),
    );
    return (projects as ProjectRow[]).flatMap((row) => {
      const customer = customerMap.get(row.customer_id);
      if (!customer) return [];
      const recent = (timeline as TimelineRow[]).find(
        (entry) =>
      ...
      return [
        {
          ...
          stage: asString(row.operations.stage, "Primer contacto"),
          score:
            typeof row.operations.score === "number"
              ? row.operations.score
              : 60,
          commercialStage: commercialStage(row.operations.commercialStage),
          origin: origin(row.operations.origin),
          notes: asString(row.operations.notes) || undefined,
          customerVersion: customer.version,
          lastCommunication: recent
            ? `${recent.human_message} · ${new Date(recent.occurred_at).toLocaleDateString("es-CL")}`
            : "Sin comunicaciones recientes",
          salesOwner: asString(memory.salesOwner, "Sin asignar"),
          nextAction: asString(
  ...
  async createWithProject(draft: ProjectDraft): Promise<Project> {
    if (!draft.type || (!draft.crmCustomerId && !draft.origin))
      throw new Error(
        "Selecciona el tipo de evento y el origen del nuevo cliente.",
      );
    if (!draft.reservationTransactionId)
      throw new Error("La reserva no tiene Transaction ID.");
    const { data, error } = await this.client.rpc(
      "create_or_resume_manual_reservation",
      { p_transaction_id: draft.reservationTransactionId, p_draft: draft },
    );
    if (error) throw error;
    const row = Array.isArray(data) ? data[0] : data;
    if (!row?.project_id || !row?.orbit_event_id)
      throw new Error(
    ...
    return {
      ...
      notes: draft.notes,
      customerVersion: 1,
      lastCommunication: "Relación creada · hoy",
      salesOwner: "Sin asignar",
      nextAction: "Realizar primer contacto",
      tags: [],
      reservationTransactionId: draft.reservationTransactionId,
      reservationResumed: Boolean(row.resumed),
    };
  }
  ...
  async update(input: CustomerMutationInput): Promise<void> {
    ...
    if (!data)
      throw new Error(
        "El cliente fue modificado por otra sesión. Recarga antes de continuar.",
      );
    ...
    await this.timeline.append({
      ...
      humanMessage: "Cliente actualizado correctamente.",
      correlationId: crypto.randomUUID(),
      previousState: `VERSION_${input.expectedVersion}`,
      newState: `VERSION_${input.expectedVersion + 1}`,
    });
  }
  async softDelete(
    customerId: string,
  ...
  private async setDeletedAt(
    ...
    await this.timeline.append({
      ...
      entityId: customerId,
      customerId,
      humanMessage: deletedAt
        ? "Cliente archivado correctamente."
        : "Cliente restaurado correctamente.",
      correlationId: crypto.randomUUID(),
      previousState: deletedAt ? "ACTIVE" : "ARCHIVED",
      newState: deletedAt ? "ARCHIVED" : "ACTIVE",
    });
  }
  private async projectContext(
    customerId: string,
  ): Promise<{ projectId?: string; orbitEventId: string }> {
    const { data, error } = await this.client
      .from("projects")
      .select("id,orbit_event_id")
    ...
    return {
      projectId: data?.id,
      orbitEventId: data?.orbit_event_id ?? `ORB-CUSTOMER-${customerId}`,
    };
  }

...
import { AlertCircle, CheckCircle2, Copy, FileSignature, History, Mail, Send } from "lucide-react";
  ...
  return <div className="space-y-5">
    ...
    {open && composer ? <MobileDialog description="Revisa exactamente quién recibirá la confirmación y su contenido antes de enviar." dismissOnOverlayClick={false} eyebrow="COMUNICACIONES CON EL CLIENTE" onClose={() => { if (!sending) setOpen(false); }} size="xl" title={actionLabel} variant="fullscreen-mobile">
      <div className="space-y-5">
        {sendState.status !== "idle" ? <div aria-live="polite" className={`rounded-2xl border p-4 text-sm ${sendState.status === "success" ? "border-emerald-300 bg-emerald-50 text-emerald-900" : sendState.status === "error" ? "border-rose-300 bg-rose-50 text-rose-900" : "border-brand/30 bg-brand/5"}`}><div className="flex gap-2">{sendState.status === "success" ? <CheckCircle2 className="mt-0.5 size-4 shrink-0"/> : sendState.status === "error" ? <AlertCircle className="mt-0.5 size-4 shrink-0"/> : <span aria-hidden="true" className="mt-0.5 size-4 shrink-0"/>}<div><p className="font-medium">{sendState.message}</p>{sendState.status === "success" && sendState.cc.length ? <p className="mt-1 text-xs">CC: {sendState.cc.join(", ")}</p> : null}</div></div></div> : null}
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Detail label="PARA" value={to}/><Detail label="Servicio" value={composer.services}/><Detail label="Duración" value={composer.duration}/><Detail label="Fecha / horario" value={`${composer.eventDate} · ${composer.eventTime}`}/>{composer.companyCommercial ? <><Detail label="Valor neto" value={money(composer.net)}/><Detail label="IVA 19%" value={money(composer.vat)}/><Detail label="Total" value={money(composer.total)}/></> : <Detail label="Valor total" value={money(composer.total)}/>}<Detail label="Abono recibido" value={money(composer.paid)}/><Detail label="Saldo pendiente" value={money(composer.balance)}/><Detail label="Lugar" value={composer.venue}/></div>

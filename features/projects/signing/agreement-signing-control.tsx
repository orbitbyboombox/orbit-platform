...
export function AgreementSigningControl({ agreementId, projectId, status }: { agreementId?: string; projectId: string; status: string }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border bg-card p-5 sm:p-6" data-customer-communications>
      ...
      <dl className="mt-5 grid gap-3 sm:grid-cols-3"><Detail label="Destinatario" value={composer?.to ?? "Cargando…"}/><Detail label="CC" value={composer?.cc.length ? composer.cc.join(", ") : "Sin CC"}/><Detail label="Último envío" value={composer?.lastAttemptAt ? chileDateTime(composer.lastAttemptAt) : "Nunca"}/></dl>

...
export async function formalizeManualReservation(input:{projectId:string;actorId:string;formalization:NonNullable<ProjectDraft["commercialFormalization"]>}){
  ...
  const {data:project,error}=await admin.from("projects").select("id,name,customer_id,orbit_event_id,project_type,event_date,event_time,location,city,operations,customers!inner(full_name,email,rut,phone,company),project_services(service_code,duration_hours,extras),project_operational_contracts(service_start_at,service_end_at),quotations(quotation_number,subtotal,discount_total,tax_total,grand_total,final_customer_price,deposit_percent,accepted_snapshot)").eq("id",input.projectId).single();

...
export async function openSigningAgreement(token: string) {
  ...
  const { data: agreement, error: agreementError } = await admin.from("agreements").select("id,status,template_version,rendered_contract,project_id,projects!inner(name,event_date,event_time,location,city,customers!inner(full_name,email),project_services(service_code,duration_hours,extras),quotations(final_customer_price))").eq("id", signing.agreement_id).single();
...
export async function confirmDigitalSignature(input: { token: string; signatureDataUrl: string; ipAddress: string; userAgent: string; suppressCustomerDelivery?: boolean }) {
  ...
  try {
    ...
    const financial=acceptedCommercialFinancialPresentation(quotation?.accepted_snapshot,{subtotal:quotation?.subtotal,discountTotal:quotation?.discount_total,taxTotal:quotation?.tax_total,grandTotal:quotation?.grand_total,finalCustomerPrice:quotation?.final_customer_price,depositPercent:quotation?.deposit_percent});

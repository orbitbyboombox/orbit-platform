...
async function openAgreementForSigning(admin: ReturnType<typeof createAdminClient>, agreementId: string) {
  const { data, error } = await admin.from("agreements").select("id,status,template_version,project_id,created_by,projects!inner(id,name,customer_id,created_by,event_date,event_time,location,city,operations,customers!inner(full_name,email,rut,phone),project_services(service_code,duration_hours,extras),quotations(quotation_number,status,official_price,discount_total,final_customer_price))").eq("id", agreementId).single();

...
export async function confirmDigitalSignature(input: { token: string; signatureDataUrl: string; ipAddress: string; userAgent: string }) {
  ...
  try {
    ...
    const accessToken = await loadGoogleWorkspaceAccessToken(); const connection = await loadGoogleWorkspaceConnection(); const plan = buildCustomerFolderPlan(customer.full_name, agreement.projects.event_date); const drive = new GoogleDriveLive(connection, new GoogleDriveApiProvider(accessToken), new SupabaseGoogleDriveFolderRepository(admin, agreement.project_id)); const rootSync = await drive.synchronizeFolderPlan(buildRootFolderPlan(), signedAt); if (!rootSync.ok) throw new Error(rootSync.error.message); const sync = await drive.synchronizeFolderPlan(plan, signedAt); if (!sync.ok) throw new Error(sync.error.message);
...
async function openAgreementForSigning(admin: ReturnType<typeof createAdminClient>, agreementId: string) {
  const { data, error } = await admin.from("agreements").select("id,status,template_version,project_id,projects!inner(id,name,customer_id,event_date,event_time,location,city,customers!inner(full_name,email),project_services(service_code,duration_hours,extras),quotations(quotation_number,status))").eq("id", agreementId).single();

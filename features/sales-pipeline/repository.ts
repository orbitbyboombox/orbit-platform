...
export async function loadSalesPipeline(): Promise<SalesPipelineData> {
  ...
  const leads = rows.map((row: RawRow): SalesLead => {
    ...
    const stage = derivePipelineStage({ explicit: row.pipeline_stage ?? opText("pipelineStage"), commercialStage: opText("commercialStage"), quotationStatus: quote?.status, reservationStatus: reservation?.status });
    ...
    return { id: row.id, customerId: row.customer_id, customerName: customer?.full_name ?? "Cliente sin nombre", company: customer?.company ?? "", eventId: row.orbit_event_id, eventName: row.name, eventType: row.project_type, eventDate: row.event_date, service: row.project_services?.[0]?.service_code ?? "Por confirmar", source: source(row.lead_source ?? opText("leadSource") ?? opText("origin")), stage, estimatedValue: row.estimated_value == null ? (opNumber("estimatedValue") == null ? (quote?.final_customer_price ?? quote?.grand_total ?? null) : opNumber("estimatedValue")) : Number(row.estimated_value), nextActionAt, nextActionType: action(row.next_action_type ?? opText("nextActionType")), followUpStatus: (row.follow_up_status ?? opText("followUpStatus")) as SalesLead["followUpStatus"] ?? followUpStatus({ stage, humanHandoff: handoff, optOut, deliveryEnabled: process.env.WHATSAPP_DELIVERY_ENABLED === "true", nextActionAt, now }), lastActivityAt: latest?.occurred_at ?? null, overdue: Boolean(nextActionAt && new Date(nextActionAt).getTime() < now.getTime()), urgency: actionUrgency(nextActionAt, now), lostReason: row.lost_reason ?? opText("lostReason") ?? null };
  });
  ...
  return { leads, counts, metrics: { newLeads: counts.NUEVO, quotes: counts.COTIZACIÓN, reservations: counts["RESERVA PENDIENTE"], won: counts.GANADO, lost: counts.PERDIDO, estimatedValue: leads.reduce((sum, lead) => sum + (lead.estimatedValue ?? 0), 0), overdue: leads.filter((lead) => lead.overdue).length } };
}

...
export async function completeAutomaticBooking(input: { token: string; submission: AutomaticBookingSubmission; ipAddress: string; userAgent: string }) {
  ...
  try {
    ...
    const { error: projectError } = await measured("project_and_event360", async () => await admin.from("projects").insert({ id: projectId, customer_id: customerId, orbit_event_id: orbitEventId, name: input.submission.customer.name.trim(), project_type: input.submission.event.type, status: "Upcoming", health: "Healthy", event_date: input.submission.event.date, event_time: input.submission.event.time, event_time_mode: "ESTIMATED", location: input.submission.event.venue, city: input.submission.event.municipality, operations: { stage: "Capacidad preliminar", commercialStage: "Waiting", reservationMethod: "AUTOMATIC", notes, durationHours: input.submission.service.hours, extras: persistedExtras, brandingFaces:input.submission.service.extras.includes("Branding")?Math.max(1,input.submission.service.brandingQuantity):0 }, finance, created_by: actorId, updated_by: actorId }));

...
export async function deliverConfirmedReservationEmail(input: { projectId: string; actorId: string; portal?: { url: string; expiresAt: string } }): Promise<{ status: "SENT" | "PENDING"; messageId?: string }> {
  ...
  const textBody = [`Hola ${customer.full_name},`, "¡Bienvenido a BOOMBOX!", "Tu reserva ha sido confirmada correctamente.", "Adjuntamos tu contrato firmado.", ...summaryRows.map(([label, value]) => `${label}: ${value}`), `Fecha: ${project.event_date}`, `Hora: ${project.event_time?.slice(0, 5) ?? "Por confirmar"}`, `Lugar del evento: ${project.location ?? "Por confirmar"}`, `Dirección: ${eventAddress}`, `Contacto operacional: ${operationalContact}`, `Portal: ${portal.url}`, driveUrl && `Documentos: ${driveUrl}`, company.website].filter(Boolean).join("\n\n");

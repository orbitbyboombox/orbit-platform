...
export function formatDigitalPhotoEventDate(value: string) {
...
export function digitalPhotoDeliveryText(
  customerName: string,
  eventDate: string,
  photoUrl: string,
) {
  const eventDateLabel = formatDigitalPhotoEventDate(eventDate);
  return [
    `Hola ${customerName},`,
    "",
    "Esperamos que te encuentres muy bien.",
    "",
    `Queremos agradecerte por elegir a BOOMBOX y permitirnos ser parte de tu evento el día ${eventDateLabel}.`,
    "",
...
export function renderDigitalPhotoDeliveryPreviewHtml(
  ...
  try {
    const photoUrl = validateDigitalPhotoDeliveryUrl(suppliedPhotoUrl);
    return renderDigitalPhotoDeliveryHtml(customerName, eventDate, photoUrl);
  } catch {
    const html = renderDigitalPhotoDeliveryHtml(
      customerName,
      eventDate,
      DIGITAL_PHOTO_PREVIEW_URL,
    );
    const activeCta = `<a href="${DIGITAL_PHOTO_PREVIEW_URL}" target="_blank" rel="noopener noreferrer" style="display:inline-block;box-sizing:border-box;max-width:100%;border-radius:12px;background:#ed7203;padding:14px 28px;font-size:15px;font-weight:800;letter-spacing:.04em;color:#ffffff;text-decoration:none">DESCARGAR FOTOS</a>`;
    const disabledCta = `<span aria-disabled="true" style="display:inline-block;box-sizing:border-box;max-width:100%;border-radius:12px;background:#ed7203;padding:14px 28px;font-size:15px;font-weight:800;letter-spacing:.04em;color:#ffffff;text-decoration:none;opacity:.55;cursor:not-allowed">DESCARGAR FOTOS</span><p style="margin:10px 0 0;font-size:12px;line-height:1.45;color:#6b7280">Agrega el enlace de las fotos para habilitar este botón.</p>`;
    return html.replace(activeCta, disabledCta);
  }
}

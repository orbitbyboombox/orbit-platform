...
export class GoogleCalendarApiProvider implements GoogleCalendarLiveProvider {
  ...
  private body(payload: GoogleCalendarEventPayload) { return { summary: payload.title, description: payload.description, location: payload.location, colorId: payload.color.googleColorId, start: { dateTime: `${payload.date}T${payload.startTime}:00`, timeZone: "America/Santiago" }, end: { dateTime: `${payload.date}T${payload.endTime}:00`, timeZone: "America/Santiago" }, extendedProperties: { private: { orbitEventId: payload.orbitEventId } } }; }

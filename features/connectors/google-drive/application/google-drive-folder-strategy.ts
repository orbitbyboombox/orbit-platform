...
export const GOOGLE_DRIVE_ROOT = "BOOMBOX ORBIT";
...
export function buildRootFolderPlan(): readonly GoogleDriveFolderPlanItem[] {
  const root = folder(GOOGLE_DRIVE_ROOT, null);
...
export function buildCustomerFolderPlan(customerName: string, eventDate: string): readonly GoogleDriveFolderPlanItem[] {
  const { year } = dateParts(eventDate, "La fecha del evento");
  const clients = `${GOOGLE_DRIVE_ROOT}/CLIENTES`;
...
export function buildAccountingFolderPlan(documentDate: string, category: string): readonly GoogleDriveFolderPlanItem[] {
  const { year, month } = dateParts(documentDate, "La fecha del documento");
  const accounting = `${GOOGLE_DRIVE_ROOT}/CONTABILIDAD`;
...
export function buildStaffFolderPlan(staffMemberName: string): readonly GoogleDriveFolderPlanItem[] {
  const staffRoot = `${GOOGLE_DRIVE_ROOT}/STAFF`;
...
export function resolveAutomaticDestination(request: GoogleDriveDestinationRequest): GoogleDriveDestination {
  ...
  if (kind in customerMap) {
    ...
    const plan = buildCustomerFolderPlan(context.customerName, context.eventDate);
  ...
  if (kind in accountingMap) {
    ...
    const plan = buildAccountingFolderPlan(context.documentDate, accountingMap[kind as keyof typeof accountingMap]);
  ...
  if (kind in operatorMap) {
    ...
    return { kind, folderPath: `${GOOGLE_DRIVE_ROOT}/STAFF/${cleanName(context.staffMemberName)}/${operatorMap[kind as keyof typeof operatorMap]}` };
  }
  ...
  return { kind, folderPath: `${GOOGLE_DRIVE_ROOT}/${fixed[kind]}` };
}

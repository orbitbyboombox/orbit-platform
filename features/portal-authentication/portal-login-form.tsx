...
export function PortalLoginForm({ type, embedded = false }: { type: "CUSTOMER" | "STAFF"; embedded?: boolean }) {
  ...
  const content = <>
    ...
    <form action={formAction} className="mt-7 space-y-4"><label className="block text-sm font-medium">RUT<input autoComplete="username" className="mt-2 min-h-12 w-full rounded-xl border bg-background px-4" name="rut" required/></label>{customer ? <label className="block text-sm font-medium">Fecha del evento<div className="relative mt-2"><CalendarDays className="pointer-events-none absolute left-4 top-3.5 size-5 text-muted"/><input className="min-h-12 w-full rounded-xl border bg-background pl-12 pr-4" name="eventDate" required type="date"/></div></label> : <label className="block text-sm font-medium">PIN<div className="relative mt-2"><KeyRound className="pointer-events-none absolute left-4 top-3.5 size-5 text-muted"/><input autoComplete="current-password" className="min-h-12 w-full rounded-xl border bg-background pl-12 pr-4 tracking-[.4em]" inputMode="numeric" maxLength={4} minLength={4} name="pin" pattern="[0-9]{4}" required type="password"/></div></label>}<button className="min-h-12 w-full rounded-xl bg-brand font-semibold text-brand-foreground transition-colors disabled:opacity-60" disabled={pending}>{pending ? "Validando…" : "Ingresar"}</button>{state?.error&&<p className="rounded-xl border border-danger/20 bg-danger/10 p-3 text-sm text-danger" role="alert">{state.error}</p>}</form>

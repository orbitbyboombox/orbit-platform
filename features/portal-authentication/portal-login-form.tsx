...
export function PortalLoginForm({ type, embedded = false }: { type: "CUSTOMER" | "STAFF"; embedded?: boolean }) {
  ...
  const content = <>
    ...
    <form action={formAction} className="mt-7 space-y-4"><label className="block text-sm font-medium">RUT<RutInput className="mt-2 min-h-12 w-full rounded-xl border bg-background px-4" /></label>{customer ? <label className="block text-sm font-medium">Fecha del evento<div className="relative mt-2"><CalendarDays className="pointer-events-none absolute left-4 top-3.5 size-5 text-muted"/><input className="min-h-12 w-full rounded-xl border bg-background pl-12 pr-4" name="eventDate" required type="date"/></div></label> : <label className="block text-sm font-medium">Contraseña<div className="relative mt-2"><KeyRound className="pointer-events-none absolute left-4 top-3.5 size-5 text-muted"/><input autoComplete="current-password" className="min-h-12 w-full rounded-xl border bg-background pl-12 pr-4" minLength={4} name="pin" required type="password"/></div></label>}{customer&&state?.events?.length ? <label className="block text-sm font-medium">Selecciona tu evento<select className="mt-2 min-h-12 w-full rounded-xl border bg-background px-3" name="projectId" required defaultValue=""><option disabled value="">Selecciona una opción</option>{state.events.map((event) => <option key={event.projectId} value={event.projectId}>{event.orbCode} · {event.time} · {event.location} · {event.service}</option>)}</select></label> : null}<button className="min-h-12 w-full rounded-xl bg-brand font-semibold text-brand-foreground transition-colors disabled:opacity-60" disabled={pending}>{pending ? "Validando…" : state?.events?.length ? "Continuar con este evento" : "Ingresar"}</button>{state?.error&&<p className="rounded-xl border border-danger/20 bg-danger/10 p-3 text-sm text-danger" role="alert">{state.error}</p>}</form>

...
export type StaffBoxAssignment = { id: string; boxId: string; boxCode: string; boxStatus: string; assignmentStatus: string; components: StaffBoxComponent[]; mediaLotId: string | null; mediaRemaining: number | null; format: string | null; lot: string | null };
...
export async function loadStaffBoxOperationsAction(projectId: string): Promise<{ ok: true; assignment: StaffBoxAssignment; roles: string[] } | { ok: false; message: string }> {
  try {
    ...
    if (error) throw error;
    const row = rows?.[0];
    if (!row) return { ok: true, roles, assignment: { id: "", boxId: "", boxCode: "", boxStatus: "", assignmentStatus: "", components: [], mediaLotId: null, mediaRemaining: null, format: null, lot: null } };
    const box = Array.isArray(row.operational_assets) ? row.operational_assets[0] : row.operational_assets;
    const [{ data: components, error: componentError }, { data: lots, error: lotError }] = await Promise.all([

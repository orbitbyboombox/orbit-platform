...
export async function loadCustomerPortalProject(access: {id:string;project_id:string;customer_id:string;expires_at:string}) {
  ...
  const [project, quotation, agreement, evidence, documents, requests, payments, uploads, contractTimeline, contractFolder, invoice, designTimeline, communications, communicationTimeline] = await Promise.all([
    ...
    admin.from("documents").select("id,document_type,drive_file_id,storage_bucket,storage_path,created_at,original_filename,mime_type,version,is_current,workflow_status,approved_at,external_tax_document_type,external_folio,external_issue_date,external_total_amount,external_document_status").eq("project_id", access.project_id).or("document_type.neq.PHOTO_STRIP_DESIGN,is_current.eq.true").is("deleted_at", null).order("created_at",{ascending:true}),

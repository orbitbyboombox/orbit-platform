...
import { CreditCard, FileText, Home, Mail, Phone, ShieldCheck } from "lucide-react";
...
export function CustomerPortalHome({ data, token }: { data: NonNullable<PortalData>; token: string }) {
  ...
  const quickAccess: Array<{ icon: ComponentType<{ className?: string }>; label: string; href: string }> = [
    ...
    { icon: CreditCard, label: "Mis Pagos", href: "#payments" }, { icon: Phone, label: "Contacto BOOMBOX", href: "#contact" },
  ];
  ...
  return <main className="min-h-screen bg-background text-foreground"><div className="mx-auto max-w-7xl space-y-6 px-4 py-5 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
    ...
    <Panel id="quick-access" title="Accesos rápidos"><nav aria-label="Accesos del Portal" className="grid grid-cols-2 gap-3 lg:grid-cols-4">{quickAccess.map(({ icon: Icon, label, href }) => <a className="flex min-h-24 flex-col items-center justify-center gap-2 rounded-2xl border border-border/80 bg-background/40 p-3 text-center text-sm font-medium transition-colors hover:border-brand/50 hover:text-brand" href={href} key={label}><Icon className="size-5"/>{label}</a>)}</nav></Panel>

...
export function CustomerContractExperience({ data, token }: { data: ContractData; token: string }) {
  ...
  const status = signed ? { label: "Firmado", variant: "success" as const } : cancelled ? { label: "Cancelado", variant: "danger" as const } : { label: "Pendiente", variant: "warning" as const };
    ...
    <div className="grid min-w-0 gap-6 p-5 sm:p-7 xl:grid-cols-[minmax(0,1.2fr)_minmax(18rem,.8fr)]">
      <div className="min-w-0 space-y-4">
        <div className="flex flex-wrap gap-3">
          <ActionButton disabled={!signed || !agreement?.signed_pdf_path} icon={Download} label="Descargar PDF" onClick={() => window.open(`${contractUrl}?download=1`, "_blank", "noopener,noreferrer")}/>
          <ActionButton disabled={!signed || !agreement?.signed_pdf_path} icon={Printer} label="Imprimir" onClick={() => window.open(contractUrl, "_blank", "noopener,noreferrer")} variant="outline"/>
        ...
        {signed && agreement?.signed_pdf_path ? <div className="overflow-hidden rounded-2xl border border-border bg-background"><iframe className="h-[34rem] w-full sm:h-[46rem]" src={`${contractUrl}#toolbar=1&navpanes=0`} title="Vista previa del contrato firmado"/></div> : <div className="flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-background/40 p-8 text-center"><FileSignature className="size-9 text-brand"/><p className="mt-4 font-semibold">Contrato {cancelled ? "cancelado" : "pendiente de firma"}</p><p className="mt-2 max-w-sm text-sm leading-6 text-muted">El PDF firmado aparecerá automáticamente aquí cuando el proceso esté completo.</p></div>}
      </div>

      <aside className="min-w-0 space-y-4">
        <Card title="Información del contrato"><dl className="space-y-3"><Row label="Cliente" value={project.customers.full_name}/><Row label="Fecha del evento" value={date(project.event_date)}/><Row label="Venue" value={project.location || "Por confirmar"}/><Row label="Servicio" value={project.project_services.map((item) => item.service_code).join(" + ") || "Por confirmar"}/><Row label="Horas" value={project.project_services.map((item) => item.duration_hours ? `${item.duration_hours} horas` : null).filter(Boolean).join(" · ") || "Por confirmar"}/><Row label="Extras" value={extras.join(" · ") || "Sin extras"}/><Row label="Transporte" value={money(quotation?.transport_total)}/><Row label="Descuento" value={quotation?.discount_total ? `−${money(quotation.discount_total)}` : money(0)}/><Row emphasis label="Total" value={money(total)}/><Row label="Reserva" value={money(reservation)}/><Row label="Saldo restante" value={money(total - reservation)}/></dl></Card>

...
export function CapacityStatusPanel({ result, loading = false, missingInputs = false, missingMessage, progressiveState, validatedAt }: { result?: CapacityResult | null; loading?: boolean; missingInputs?: boolean; missingMessage?: string; progressiveState?: ProgressiveAvailabilityState; validatedAt?: string }) {
  ...
  const copy = state === "MISSING" ? { title: "Completa fecha, horario y ubicación", tone: "neutral" } : state === "LOADING" ? { title: "VALIDANDO DISPONIBILIDAD…", tone: "neutral" } : state === "AVAILABLE" ? { title: progressiveState ? progressiveAvailabilityHeadline(state) : "DISPONIBLE ✓", tone: "success" } : state === "UNAVAILABLE" ? { title: progressiveAvailabilityHeadline(state), tone: "danger" } : { title: progressiveAvailabilityHeadline(state), tone: state === "REVIEW_REQUIRED" ? "warning" : "neutral" };

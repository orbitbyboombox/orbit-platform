...
export function PdfViewer({ title, src, onClose }: { title: string; src: string; onClose: () => void }) {
  ...
  return <div className="pointer-events-auto fixed inset-0 z-[100] flex flex-col bg-background" role="dialog" aria-modal="true" aria-label={title} onClick={(event) => event.stopPropagation()}>
    <header className="flex min-h-16 items-center gap-2 border-b bg-card px-3 sm:px-5">
      <Button onClick={close} onPointerUp={close} variant="ghost"><ArrowLeft />Volver a ORBIT</Button>
      ...
      <Button aria-label="Cerrar visor" onClick={close} onPointerUp={close} size="icon" variant="ghost"><X /></Button>
    </header>

          ...
          visibleQuotes.map((q) => (
            ...
            >
              ...
              <div className="flex min-w-0 flex-wrap gap-3 lg:justify-end">
                ...
                {["SENT","VIEWED"].includes(q.status) ? <Button className="min-h-11 px-2" loading={pending} loadingLabel="GUARDANDO ACEPTACIÓN…" onClick={() => { if(!window.confirm(`¿Confirmar que ${q.number} fue aceptada por el cliente?`))return; startTransition(async()=>{const result=await acceptCommercialQuoteAction(q.id);setMessage(result.ok?result.message:result.error);if(result.ok)window.location.reload()})}} variant="ghost">MARCAR COMO ACEPTADA</Button>:null}
                {q.status==="ACCEPTED" ? <Button className="min-h-11 px-2" loading={pending} loadingLabel="PREPARANDO RESERVA…" onClick={()=>startTransition(async()=>{const result=await loadCommercialQuoteConversionReviewAction(q.id);if(!result.ok){setMessage(result.error);return}if(result.converted){window.location.assign(`/projects/${result.projectId}`);return}setReview(result.review)})} variant="ghost">GENERAR RESERVA DESDE COTIZACIÓN</Button>:null}
...
function SendHistory({ sends }: { sends: CommercialHubData["recentSends"] }) {
  const [expanded, setExpanded] = useState(false);
  const sent = sends.filter((send) => send.status === "SENT").length;
  const pending = sends.filter((send) => send.status !== "SENT").length;
  const failed = sends.filter((send) => /FAIL|ERROR/i.test(send.status)).length;
  return <section className="rounded-2xl border bg-card p-5"><h2 className="font-semibold">Historial de envíos</h2><div className="mt-3 grid gap-3 text-xs sm:grid-cols-4"><div><span className="text-muted">Último envío</span><p className="mt-1 font-semibold">{sends[0]?.sentAt?.slice(0, 16).replace("T", " ") ?? "—"}</p></div><div><span className="text-muted">Enviados</span><p className="mt-1 font-semibold">{sent}</p></div><div><span className="text-muted">Pendientes</span><p className="mt-1 font-semibold">{pending}</p></div><div><span className="text-muted">Fallidos</span><p className="mt-1 font-semibold">{failed}</p></div></div><button className="mt-4 rounded-lg border px-3 py-2 text-xs font-semibold text-brand" onClick={() => setExpanded((value) => !value)}>{expanded ? "Ocultar envíos" : "Ver envíos"}</button>{expanded && <div className="mt-4 max-h-80 divide-y overflow-y-auto">{sends.length ? sends.map((send) => <div className="grid min-w-0 gap-1 py-3 text-sm sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] sm:gap-4" key={send.id}><div className="min-w-0"><p className="break-all font-medium">Para: {send.recipient}</p><p className="break-all text-xs text-muted">CC: {send.ccRecipients.length ? send.ccRecipients.join(", ") : "Sin CC"}</p></div><div className="min-w-0"><p className="truncate">{send.subject}</p></div><span className="text-muted">{send.status} · {send.sentAt.slice(0, 10)}</span></div>) : <p className="py-5 text-sm text-muted">Aún no hay envíos comerciales.</p>}</div>}</section>;
}
function Field({
  label,
  children,

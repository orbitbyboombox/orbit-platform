...
export function simulateBiancaMessage(message: string): BiancaSimulationResult {
  ...
  const hasName = /\b(?:soy|me llamo|mi nombre es|hablo con)\s+[A-Za-zÁÉÍÓÚáéíóúÑñ]+|\bmat[ií]as\b/i.test(text);
  const hasCommune = /\b(?:comuna\s+de\s+)?(?:pudahuel|lo\s+barnechea|santiago|maip[uú]|las\s+condes|providencia|colina|puente\s+alto|pirque)\b/i.test(text);
  const hasVenue = /\b(?:centro|recinto|sal[oó]n|alto\s+noviciado|santa\s+martina|club\s+valle|casona)\b/i.test(text);
  const missing = [!hasName && "nombre", !date(text) && "fecha", !time(text) && "hora", !hasCommune && "comuna", hasCommune && !hasVenue && "lugar o centro de eventos"].filter(Boolean) as string[];
  return { intent, customerState: "NEW_CUSTOMER", extracted, missing, sources: [...sources, ...(intent === "MATRIMONIO" ? ["Matrimonio catalog/rules (fixture)"] : ["Commercial catalog/rules (fixture)"])], proposedResponse: missing.length ? "¡Hola! 😊 Claro, te ayudo. Para revisar disponibilidad, ¿me confirmas " + missing.slice(0, 2).join(" y ") + "?" : "¡Buenísimo! Ya tengo los datos principales. Voy a revisar la información oficial de BOOMBOX.", proposedAction: missing.length ? "COLLECT_MINIMUM_DATA" : "CANONICAL_COMMERCIAL_LOOKUP", escalation: false, controlState: "BIANCA_ACTIVE", founderAlert: false, externalFounderWhatsApp: "OFF" };
}

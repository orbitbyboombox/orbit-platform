...
export interface OperationalResource { id:string; source:ResourceSource; category:ResourceCategory; name:string; code:string; status:string; enabled:boolean; version:number; serialNumber?:string|null; storageLocation?:string|null; notes?:string|null; manufacturer?:string|null; model?:string|null; currentAssignment?:{projectId:string;eventName:string|null;eventDate:string|null;eventTime:string|null}|null; }
...
function asset(row:{id:string;asset_code:string;status:string;version:number;metadata:unknown;serial_number?:string|null;storage_location?:string|null;notes?:string|null;manufacturer?:string|null;model?:string|null},category:ResourceCategory):OperationalResource{const metadata=(row.metadata??{})as Record<string,unknown>;return{id:row.id,source:"ASSET",category,name:typeof metadata.name==="string"?metadata.name:row.asset_code,code:row.asset_code,status:row.status,enabled:row.status!=="OUT_OF_SERVICE",version:row.version,serialNumber:row.serial_number??null,storageLocation:row.storage_location??null,notes:row.notes??null,manufacturer:row.manufacturer??null,model:row.model??null};}
...
export async function createResourceAction(formData:FormData):Promise<Result>{
  try{
    ...
    if(source==="ASSET"){
      ...
      const{data,error}=await client.from("operational_assets").insert({asset_code:code,asset_type:assetType,status:"AVAILABLE",qr_key:`orbit:asset:${code}`,metadata:{name,resourceCategory:category},serial_number:value(formData,"serialNumber")||null,storage_location:value(formData,"storageLocation")||null,notes:value(formData,"notes")||null,manufacturer:value(formData,"manufacturer")||null,model:value(formData,"model")||null,created_by:userId,updated_by:userId}).select("id,asset_code,status,version,metadata,serial_number,storage_location,notes,manufacturer,model").single();if(error)throw error;return{ok:true,item:asset(data,category)};
    }
    if(source==="SUPPLY"){
...
export async function updateResourceAction(formData:FormData):Promise<Result>{
  try{
    ...
    if(source==="ASSET"){
      ...
      const assetType=category==="VEHICLES"?"VEHICLE":"ACCESSORY";const{data,error}=await client.from("operational_assets").update({asset_code:code,asset_type:assetType,qr_key:`orbit:asset:${code}`,metadata:{...((current.metadata??{})as Record<string,unknown>),name,resourceCategory:category},serial_number:value(formData,"serialNumber")||null,storage_location:value(formData,"storageLocation")||null,notes:value(formData,"notes")||null,manufacturer:value(formData,"manufacturer")||null,model:value(formData,"model")||null,updated_by:userId}).eq("id",id).eq("version",version).is("deleted_at",null).select("id,asset_code,status,version,metadata,serial_number,storage_location,notes,manufacturer,model").maybeSingle();if(error)throw error;if(!data)throw new Error("El recurso cambió en otra sesión.");return{ok:true,item:asset(data,category)};
    }
    if(source==="SUPPLY"){
...
export async function setResourceEnabledAction(item:OperationalResource,enabled:boolean):Promise<Result>{
  try{
    ...
    if(item.source==="ASSET"){const status=enabled?"AVAILABLE":"OUT_OF_SERVICE";const{data,error}=await client.from("operational_assets").update({status,updated_by:userId}).eq("id",item.id).eq("version",item.version).is("deleted_at",null).select("id,asset_code,status,version,metadata,serial_number,storage_location,notes,manufacturer,model").maybeSingle();if(error)throw error;if(!data)throw new Error("El recurso cambió en otra sesión.");return{ok:true,item:asset(data,item.category)};}

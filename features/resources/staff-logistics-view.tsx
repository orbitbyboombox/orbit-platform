...
function LogisticsMobileCard({ event, sector, onSelect, selected }: { event: StaffLogisticsEvent; sector: LogisticsSector; onSelect: () => void; selected: boolean }) {
  const state = statusView[normalizeStatus(event.status)];
  const date = dateParts(event.date);
  return <button className={`grid w-full min-w-0 grid-cols-[54px_4px_minmax(0,1fr)_auto_16px] items-center gap-2 rounded-2xl border border-white/10 bg-[#111214] px-2.5 py-2 text-left ${selected ? "border-brand/60 bg-brand/5" : ""}`} onClick={onSelect}><span className="flex min-h-[64px] flex-col items-center justify-center px-0.5"><strong className="text-[10px] font-bold leading-none text-white/60">{date.weekday}</strong><strong className="mt-1 text-xl leading-none text-white">{date.day}</strong><span className="mt-1 text-[9px] font-semibold leading-none text-white/40">{date.month}</span></span><span className={`h-14 w-1 rounded-full ${state.bar}`} /><span className="min-w-0"><strong className="block truncate text-sm leading-4 text-white">{event.customer}</strong><span className="mt-0.5 block truncate text-[11px] leading-4 text-white/75">{formatTime(event.time)} → {formatTime(event.endTime)} · {event.service}{event.duration ? ` · ${event.duration}h` : ""}</span><span className="mt-0.5 block truncate text-[10px] leading-4 text-white/45">{event.location} · {event.commune} · {sector}</span><span className="mt-0.5 block truncate text-[10px] leading-4 text-white/40">Citación {formatTime(event.staffCallAt)} · {event.operator} · {event.box}</span></span><span className="flex min-w-0 flex-col items-end gap-1"><span className={`inline-flex max-w-20 items-center gap-1 truncate text-[10px] font-medium ${state.color}`}><i className={`size-1.5 shrink-0 rounded-full ${state.dot}`} />{state.label}</span><span className="max-w-20 truncate rounded-full border border-white/10 bg-white/[.05] px-1.5 py-0.5 text-[9px] font-semibold text-white/70">{event.service}</span></span><ChevronRight className="size-4 text-brand" /></button>;
}

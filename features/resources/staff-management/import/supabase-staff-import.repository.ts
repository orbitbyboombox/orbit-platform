...
export class SupabaseStaffImportRepository {
  ...
  async import(rows: readonly StaffImportRow[]): Promise<number> {
    ...
    const payload = rows.map((row) => ({ employeeCode: row.employeeCode, firstName: row.firstName, lastName: row.lastName, rut: row.rut, phone: row.phone, email: row.email, status: row.status, roleClassification: row.roleClassification, capabilities: row.capabilities, specializations: row.specializations, notes: row.notes, bank: row.bank, accountNumber: row.accountNumber, emergencyContact: row.emergencyContact }));

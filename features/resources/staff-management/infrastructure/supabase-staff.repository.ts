...
export class SupabaseStaffRepository implements StaffRepository {
  ...
  async findAll(): Promise<readonly StaffMember[]> {
    const [{ data: staff, error: staffError }, { data: assignments, error: assignmentError }, { data: projects, error: projectError }] = await Promise.all([
      this.client.from("staff").select("id,first_name,last_name,rut,phone,email,address,commune,emergency_contact,start_date,role,rates,driving_license,can_drive,availability,observations,status,version").is("deleted_at", null).order("last_name"),
  ...
  private toMember(row: StaffRow, assignments: AssignmentRow[], projects: Map<string, ProjectRow>): StaffMember {
    ...
    return {
      ...
      employment: { staffType: staffType(row.role), dailyEventRate: operatorCost, installationRate: installationCost, removalRate: removalCost, drivingLicense: row.driving_license ?? undefined, canDriveCompanyVehicle: row.can_drive, availability: text(row.availability?.label, "Sin disponibilidad informada"), observations: row.observations ?? undefined },

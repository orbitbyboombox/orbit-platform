...
import {
  addFuelLogAction,
  assignVehicleToEventAction,
  createVehicleAction,
  deleteVehicleAction,
  setVehicleEnabledAction,
  updateVehicleAction,
  type FleetVehicle,
  type FuelLog,
} from "./fleet-center.actions";
...
type Panel =
  | { kind: "create" }
  | { kind: "edit" | "fuel" | "assign"; vehicle: FleetVehicle }
  | null;
...
const fuelLabel = (value: string) =>
  value === "GASOLINE_93" ? "Gasolina · 93 octanos" : "Diésel";
...
export function FleetCenter({
  ...
  const save = (data: FormData, editing: boolean) => {
    ...
    startTransition(async () => {
      const result = editing
        ? await updateVehicleAction(data)
        : await createVehicleAction(data);
      if (!result.ok) return setError(result.error);
      setVehicles((current) =>
        editing
          ? current.map((item) =>
              item.id === result.vehicle.id ? result.vehicle : item,
            )
  ...
  const toggle = (vehicle: FleetVehicle) => {
    setError("");
    startTransition(async () => {
      const result = await setVehicleEnabledAction(vehicle, !vehicle.enabled);
      if (!result.ok) return setError(result.error);
      setVehicles((current) =>
  ...
  const remove = (vehicle: FleetVehicle) => {
    if (!window.confirm(`¿Eliminar ${vehicle.model} de la flota activa?`))
      return;
    setError("");
    startTransition(async () => {
      const result = await deleteVehicleAction(vehicle);
      if (!result.ok) return setError(result.error);
      setVehicles((current) =>
  ...
  const addFuel = (data: FormData) => {
    setError("");
    startTransition(async () => {
      const result = await addFuelLogAction(data);
      if (!result.ok) return setError(result.error);
      setLogs((current) => [result.log, ...current]);
      setVehicles((current) =>
        current.map((item) =>
          item.id === result.log.assetId
            ? { ...item, fuelTotal: item.fuelTotal + result.log.totalAmount }
            : item,
        ),
      );
      setPanel(null);
  ...
  const assign = (data: FormData) => {
    setError("");
    startTransition(async () => {
      const result = await assignVehicleToEventAction(data);
      if (!result.ok) return setError(result.error);
      setPanel(null);
    });
  };
  return (
    <section className="space-y-5" aria-labelledby="fleet-title">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-medium text-brand">FLOTA BOOMBOX</p>
          <h2
            id="fleet-title"
            className="mt-2 text-2xl font-semibold sm:text-3xl"
          >
            Vehículos operacionales
          </h2>
          <p className="mt-2 text-sm text-muted">
            Flota, combustible y asignaciones vinculadas a costos operacionales.
          </p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
        {vehicles.map((vehicle) => {
          const vehicleLogs = logs.filter((log) => log.assetId === vehicle.id);
          return (
            <article
              key={vehicle.id}
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">
                    {vehicle.nickname || "Sin apodo"} ·{" "}
                    {vehicle.plate || "Patente pendiente"}
                  </p>
                </div>
                <span
                  className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${vehicle.enabled ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-600" : "border-red-500/30 bg-red-500/10 text-red-600"}`}
                >
                  {vehicle.enabled ? "Operacional" : "Deshabilitado"}
                </span>
              </div>
                ...
                <Detail
                  ...
                  value={
                    vehicle.currentMileage === null
                      ? "Pendiente"
                      : `${vehicle.currentMileage.toLocaleString("es-CL")} km`
                  }
                />
                <Detail
                  label="Seguro"
                  value={vehicle.insuranceExpiration || "Pendiente"}
                />
                <Detail
              ...
              <div className="mt-5 grid grid-cols-2 gap-2 border-t pt-4 sm:grid-cols-3">
                <Action
                  icon={<Pencil />}
                  label="Editar"
                  onClick={() => setPanel({ kind: "edit", vehicle })}
                />
                {showFuelControl && <Action
                  icon={<Fuel />}
                  label="Agregar combustible"
                  onClick={() => setPanel({ kind: "fuel", vehicle })}
                />}
                <Action
                  icon={<Route />}
                  label="Asignar a evento"
                  onClick={() => setPanel({ kind: "assign", vehicle })}
                />
                <Action
                  icon={<Power />}
...
function FleetPanel({
  ...
  onFuel: (data: FormData) => void;
  onAssign: (data: FormData) => void;
}) {
  const vehicle = panel.kind === "create" ? null : panel.vehicle;
  return (
    <div
    ...
    >
      <div className="max-h-[92dvh] w-full overflow-y-auto rounded-t-2xl border bg-card p-5 sm:max-w-2xl sm:rounded-2xl sm:p-7">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-semibold text-brand">FLOTA</p>
...
function VehicleForm({
  ...
  return (
    <form
      action={(data) => onSubmit(data, !!vehicle)}
    ...
    >
      ...
      <Field
        ...
        required
      />
      <Field label="Apodo" name="nickname" defaultValue={vehicle?.nickname} />
      <Field label="Patente" name="plate" defaultValue={vehicle?.plate} />
      <Select
        label="Combustible"
        name="fuelType"
        defaultValue={vehicle?.fuelType ?? "GASOLINE_93"}
        options={[
          { value: "GASOLINE_93", label: "Gasolina · 93 octanos" },
      ...
      <Field
        ...
        name="currentMileage"
        type="number"
        defaultValue={vehicle?.currentMileage?.toString()}
      />
      <Field
        label="Vencimiento seguro"
        name="insuranceExpiration"
        type="date"
        defaultValue={vehicle?.insuranceExpiration}
      />
      <Field
        label="Vencimiento revisión técnica"
        name="technicalInspectionExpiration"
        type="date"
        defaultValue={vehicle?.technicalInspectionExpiration}
      />
      <Select
        label="Estado operacional"
        name="operationalStatus"
        defaultValue={vehicle?.operationalStatus ?? "OPERATIONAL"}
        options={[
...
function FuelForm({
  ...
  return (
    <form action={onSubmit} className="mt-6 grid gap-4 sm:grid-cols-2">
      <input type="hidden" name="assetId" value={vehicle.id} />
      ...
      />
      <Field label="Litros" name="litres" type="number" step="0.001" required />
      <Field label="Monto total" name="totalAmount" type="number" required />
      <Field label="Estación de servicio" name="gasStation" required />
      <label className="text-sm font-medium">
        ...
        <input
          ...
          className="mt-2 block w-full rounded-xl border bg-background p-2 text-sm"
        />
      </label>
      {error && <p className="sm:col-span-2 text-sm text-red-600">{error}</p>}
      <Button className="sm:col-span-2" disabled={pending}>
...
function AssignmentForm({
  ...
  return (
    <form action={onSubmit} className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      />
      <Field label="Ruta" name="route" required />
      <Field
        label="Distancia (km)"
        name="distance"
        type="number"

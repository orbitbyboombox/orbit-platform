...
const categories: readonly { value: EquipmentCategory; label: string }[] = [
  ...
  { value: "DISPLAY_22", label: 'Display lateral 22"' },
...
function EquipmentPanel({ panel, historyEntries, pending, error, onClose, onSave }: { panel: Exclude<Panel, null>; historyEntries: EquipmentHistoryEntry[]; pending: boolean; error: string; onClose: () => void; onSave: (data: FormData, editing: boolean) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-6" role="dialog" aria-modal="true">
    <div className="max-h-[92dvh] w-full overflow-y-auto rounded-t-2xl border bg-card p-5 shadow-2xl sm:max-w-xl sm:rounded-2xl sm:p-7">
      ...
      {panel.kind === "history" && item ? <div className="mt-6 space-y-3">{historyEntries.filter((entry) => entry.assetId === item.id).map((entry) => <div key={entry.id} className="rounded-xl border p-4"><p className="text-sm font-medium">{entry.message}</p><p className="mt-1 text-xs text-muted">{new Intl.DateTimeFormat("es-CL", { dateStyle: "medium", timeStyle: "short" }).format(new Date(entry.occurredAt))}</p></div>)}{!historyEntries.some((entry) => entry.assetId === item.id) && <p className="rounded-xl border border-dashed p-6 text-center text-sm text-muted">Este equipo aún no registra movimientos.</p>}</div> : panel.kind === "view" && item ? <dl className="mt-6 grid gap-4 sm:grid-cols-2"><Detail label="Nombre" value={item.name} /><Detail label="Código" value={item.code} /><Detail label="Categoría" value={categoryLabel(item)} /><Detail label="Estado" value={statusLabel(item.status)} /><Detail label="Usos" value={String(item.usageCount)} /><Detail label="Serial" value={item.serialNumber??"No registrado"} /><Detail label="Ubicación" value={item.storageLocation??"No registrada"} /></dl> : <form className="mt-6 space-y-4" action={(data) => onSave(data, editing)}>
...
function Field({ label, name, defaultValue, required=true }: { label: string; name: string; defaultValue?: string; required?: boolean }) { return <label className="block text-sm font-medium">{label}<input required={required} name={name} defaultValue={defaultValue} className="mt-2 h-11 w-full rounded-xl border bg-background px-3 outline-none focus:border-brand" /></label>; }

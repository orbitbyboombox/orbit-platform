...
export function CommandCenter({ readiness, availableOperators, availableTotems, availableCases, availablePrinters, availableCameras, taskSummary, executive }: { readiness:readonly CommandCenterProjectReadiness[]; availableOperators:number; availableTotems:number; availableCases:number; availablePrinters:number; availableCameras:number; taskSummary:{pending:number;critical:number;overdue:number;today:number}; executive:{next15Events:number;accountsReceivable:number;monthlyRevenue:number;monthlyGoal:number} }) {
  ...
  const upcomingEventsHref = firstTodayEvent ? `/projects/${firstTodayEvent.projectId}#event-readiness` : "/projects?focus=upcoming";
  const todayEventHref = firstTodayEvent ? `/projects/${firstTodayEvent.projectId}#event-readiness` : "/projects?focus=upcoming";
  const pendingContractsHref = firstPendingContract ? `/projects/${firstPendingContract.projectId}#agreement-control` : "/projects?filter=pending-signature";
  ...
  return <div className="space-y-9 lg:space-y-11">
    ...
    <section aria-label="Resumen operacional" className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
      ...
      <Metric href="/finance/receivables?status=outstanding" icon={CircleDollarSign} label="Cuentas por cobrar" tone="warning" value={pendingPayments} />
      <EquipmentMetric cameras={availableCameras} cases={availableCases} printers={availablePrinters} totems={availableTotems} />
      <Metric href="/resources/staff#today-staff" icon={UserCheck} label="Staff disponible" tone="success" value={availableOperators} />
      ...
      <Metric href="/tasks?status=PENDING" icon={ListChecks} label="Tareas pendientes" tone={taskSummary.pending ? "warning" : "success"} value={taskSummary.pending} />

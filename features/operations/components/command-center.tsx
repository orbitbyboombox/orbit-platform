...
import { AlertTriangle, CalendarDays, Camera, CarFront, CheckCircle2, CircleDollarSign, Clock3, FileSignature, FolderKanban, ListChecks, PackageCheck, Plus, ReceiptText, ShieldAlert, Sparkles, UserCheck } from "lucide-react";
...
function dashboardHref(path:string) {
  ...
  return `${base}${separator}from=dashboard${hash ? `#${hash}` : ""}`;
}
...
function EventReadinessCard({ item }: { item:CommandCenterProjectReadiness }) {
  ...
  return <article className="rounded-2xl border bg-card p-5 sm:p-6">
    ...
    <div className="mt-5 flex items-center justify-between gap-4"><p className="text-xs text-muted">Panel informativo. La operación nunca se bloquea automáticamente.</p><Button onClick={() => router.push(dashboardHref(`/projects/${item.projectId}#event-readiness`))} variant="outline">Abrir evento</Button></div>
  </article>;
}

function Metric({ href, label, value, icon:Icon, tone="neutral" }: { href:string; label:string; value:number|string; icon:typeof CalendarDays; tone?:"neutral"|"success"|"warning"|"danger" }) {
  const router = useRouter();
...
function EquipmentMetric({ cases, cameras, printers, totems }: { cases:number; cameras:number; printers:number; totems:number }) {
  const router = useRouter();
  return <button aria-label="Abrir Equipamiento disponible" className="rounded-2xl border bg-card p-4 text-left transition hover:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/50 sm:p-5" onClick={() => router.push(dashboardHref("/resources?status=AVAILABLE#equipment-title"))}><div className="flex items-center justify-between"><PackageCheck aria-hidden="true" className="size-4 text-success"/><span className="text-2xl font-semibold tabular-nums">{totems + cases + printers + cameras}</span></div><p className="mt-3 text-xs leading-5 text-muted sm:text-sm">Equipamiento disponible</p><p className="mt-2 text-[0.6875rem] leading-5 text-muted">{totems} tótems · {cases} cases<br/>{printers} impresoras · {cameras} cámaras</p></button>;
}

export function CommandCenter({ readiness, availableOperators, availableTotems, availableCases, availablePrinters, availableCameras, taskSummary, executive,staffIntelligence }: { readiness:readonly CommandCenterProjectReadiness[]; availableOperators:number; availableTotems:number; availableCases:number; availablePrinters:number; availableCameras:number; taskSummary:{pending:number;critical:number;overdue:number;today:number}; executive:{next15Events:number;accountsReceivable:number;monthlyRevenue:number;monthlyExpenses:number;availableVehicles:number;projectCount:number;monthlyGoal:number};staffIntelligence:{mostActiveOperator:string;mostActiveAssembly:string;totalStaffCost:number;averageCostPerEvent:number;upcomingAssignments:number} }) {
  const router = useRouter();
  ...
  const firstPendingContract = readiness.find((item) => item.statuses.some((status) => (status.label === "Acuerdo" || status.label === "Firma") && status.state !== "READY"));
  const upcomingEventsHref = dashboardHref("/projects?focus=upcoming");
  const todayEventHref = firstTodayEvent ? dashboardHref(`/projects/${firstTodayEvent.projectId}#event-readiness`) : dashboardHref("/projects?focus=today");
  const pendingContractsHref = firstPendingContract ? dashboardHref(`/projects/${firstPendingContract.projectId}#agreement-control`) : dashboardHref("/projects?filter=pending-signature");
  ...
  return <div className="space-y-9 lg:space-y-11">
    ...
    <section aria-label="Resumen ejecutivo" className="grid grid-cols-2 gap-3 lg:grid-cols-4"><button className="rounded-2xl border bg-card p-5 text-left transition hover:border-brand" onClick={()=>router.push(upcomingEventsHref)}><p className="text-2xl font-semibold">{executive.next15Events}</p><p className="mt-1 text-sm font-medium">Próximos 15 eventos</p></button><button className="rounded-2xl border bg-card p-5 text-left transition hover:border-brand" onClick={()=>router.push(dashboardHref("/finance/receivables?status=outstanding"))}><p className="text-2xl font-semibold">{executive.accountsReceivable.toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0})}</p><p className="mt-1 text-sm font-medium">Cuentas por cobrar</p></button><button className="rounded-2xl border bg-card p-5 text-left transition hover:border-brand" onClick={()=>router.push(dashboardHref("/reports?period=month&metric=revenue"))}><p className="text-2xl font-semibold">{executive.monthlyRevenue.toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0})}</p><p className="mt-1 text-sm font-medium">Ingresos del mes</p></button><button className="rounded-2xl border bg-card p-5 text-left transition hover:border-brand" onClick={()=>router.push(dashboardHref("/settings#master-data"))}><p className="text-2xl font-semibold">{executive.monthlyGoal?executive.monthlyGoal.toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0}):"Sin definir"}</p><p className="mt-1 text-sm font-medium">Meta mensual</p></button></section>

    <section aria-label="Inteligencia de Staff" className="rounded-2xl border bg-card p-5 sm:p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-brand">Staff Intelligence</p><h2 className="mt-2 text-lg font-semibold">Equipo operacional</h2></div><Button onClick={()=>router.push(dashboardHref("/resources/staff"))} variant="outline">Abrir Staff</Button></div><dl className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5"><DetailMetric label="Operador más activo" value={staffIntelligence.mostActiveOperator}/><DetailMetric label="Montaje más activo" value={staffIntelligence.mostActiveAssembly}/><DetailMetric label="Costo total Staff" value={staffIntelligence.totalStaffCost.toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0})}/><DetailMetric label="Promedio por evento" value={staffIntelligence.averageCostPerEvent.toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0})}/><DetailMetric label="Próximas asignaciones" value={String(staffIntelligence.upcomingAssignments)}/></dl></section>

    <section aria-label="Resumen operacional" className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
      <Metric href={todayEventHref} icon={CalendarDays} label="Eventos de hoy" value={todayEvents.length} />
      <Metric href={pendingContractsHref} icon={FileSignature} label="Contratos pendientes" tone="warning" value={pendingContracts} />
      <Metric href={dashboardHref("/finance/receivables?status=outstanding")} icon={CircleDollarSign} label="Cuentas por cobrar" tone="warning" value={pendingPayments} />
      <EquipmentMetric cameras={availableCameras} cases={availableCases} printers={availablePrinters} totems={availableTotems} />
      <Metric href={dashboardHref("/resources/staff#today-staff")} icon={UserCheck} label="Staff disponible" tone="success" value={availableOperators} />
      <Metric href={dashboardHref("/resources#fleet-title")} icon={CarFront} label="Vehículos" tone="success" value={executive.availableVehicles} />
      <Metric href={dashboardHref("/finance?view=expenses&period=month")} icon={ReceiptText} label="Gastos del mes" value={executive.monthlyExpenses.toLocaleString("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0})} />
      <Metric href={dashboardHref("/notifications")} icon={AlertTriangle} label="Notificaciones" tone={alerts.length ? "danger" : "success"} value={alerts.length} />
      <Metric href={dashboardHref("/projects")} icon={FolderKanban} label="Proyectos" value={executive.projectCount} />
      <Metric href={dashboardHref("/projects?status=ready")} icon={CheckCircle2} label="Eventos ready" tone="success" value={readyEvents} />
      <Metric href={dashboardHref("/projects?status=blocked")} icon={ShieldAlert} label="Eventos bloqueados" tone={blockedEvents ? "danger" : "success"} value={blockedEvents} />
      <Metric href={dashboardHref("/tasks?status=PENDING")} icon={ListChecks} label="Tareas pendientes" tone={taskSummary.pending ? "warning" : "success"} value={taskSummary.pending} />
      <Metric href={dashboardHref("/tasks?priority=CRITICAL")} icon={AlertTriangle} label="Tareas críticas" tone={taskSummary.critical ? "danger" : "success"} value={taskSummary.critical} />
      <Metric href={dashboardHref("/tasks?status=OVERDUE")} icon={Clock3} label="Tareas vencidas" tone={taskSummary.overdue ? "danger" : "success"} value={taskSummary.overdue} />
      <Metric href={dashboardHref("/tasks?due=today")} icon={CalendarDays} label="Tareas de hoy" tone={taskSummary.today ? "warning" : "success"} value={taskSummary.today} />
    </section>

    <section className="rounded-2xl border bg-card p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand">Trabajo pendiente</p><p className="mt-2 text-lg font-semibold">{taskSummary.pending ? `${taskSummary.pending} tareas requieren seguimiento.` : "No hay tareas pendientes."}</p><p className="mt-1 text-sm text-muted">El Centro de Tareas organiza el trabajo futuro; Timeline conserva el historial.</p></div><Button onClick={() => router.push(dashboardHref("/tasks?status=PENDING"))} variant="outline">Abrir tareas</Button></div></section>

    {firstAlert ? <section className="rounded-2xl border border-brand/25 bg-card p-5 sm:p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><Sparkles aria-hidden="true" className="mt-0.5 size-5 shrink-0 text-brand" /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand">ORBIT NOVA · recomendación operacional</p><p className="mt-2 text-lg font-semibold">Revisar preparación de {firstAlert.customerName}.</p><p className="mt-1 text-sm text-muted">Existe información pendiente antes del evento.</p></div></div><Button onClick={() => router.push(dashboardHref(`/projects/${firstAlert.projectId}#event-readiness`))}>Resolver ahora</Button></div></section> : <button className="w-full rounded-2xl border bg-card p-5 text-left transition hover:border-brand sm:p-6" onClick={()=>router.push(dashboardHref("/projects?status=ready"))}><p className="font-semibold">Todo está listo para operar.</p><p className="mt-1 text-sm text-muted">No existen alertas operacionales pendientes. Abrir eventos listos.</p></button>}

    <section className="space-y-5"><SectionTitle description="Agenda operacional construida con eventos productivos de la jornada." title="Eventos de hoy" /><div className="grid gap-4 lg:grid-cols-2">{todayEvents.map((item) => <EventReadinessCard item={item} key={item.projectId} />)}{!todayEvents.length && <div className="rounded-2xl border bg-card p-6"><p className="font-semibold">No hay eventos programados para hoy.</p><p className="mt-2 text-sm text-muted">Revisa los próximos eventos para anticipar la operación.</p><Button className="mt-5" onClick={() => router.push(dashboardHref("/projects?focus=upcoming"))}>Abrir próximos eventos</Button></div>}</div></section>
...
function DetailMetric({label,value}:{label:string;value:string}){return <div className="rounded-xl border bg-background/30 p-3"><dt className="text-xs text-muted">{label}</dt><dd className="mt-2 text-sm font-semibold">{value}</dd></div>}

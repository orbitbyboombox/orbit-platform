...
export function CommandCenter({ readiness, availableOperators, availableTotems, availableCases, availablePrinters, availableCameras, taskSummary, executive }: { readiness:readonly CommandCenterProjectReadiness[]; availableOperators:number; availableTotems:number; availableCases:number; availablePrinters:number; availableCameras:number; taskSummary:{pending:number;critical:number;overdue:number;today:number}; executive:{next15Events:number;accountsReceivable:number;monthlyRevenue:number;monthlyExpenses:number;availableVehicles:number;projectCount:number;monthlyGoal:number} }) {
  const router = useRouter();

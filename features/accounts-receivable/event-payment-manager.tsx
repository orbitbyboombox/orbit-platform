      ...
      {action && (
        ...
        >
          <form action={submit} className="space-y-4">
            ...
            <Button className="w-full" aria-busy={pending} disabled={pending}>
      ...
      {receiptAttachment && (
        <MobileDialog dismissOnOverlayClick={false} eyebrow="Payment Ledger" onClose={() => { if (!pending) setReceiptAttachment(null); }} size="lg" title="Adjuntar comprobante al pago existente">
          <form action={(data) => { data.set("invoiceId", receivable.id); data.set("projectId", projectId); data.set("paymentId", receiptAttachment.id); startTransition(async () => { const result = await attachReceivablePaymentReceiptAction(data); setFeedback(result.ok ? result.message ?? "Comprobante adjuntado sin modificar el pago." : result.error); if (result.ok) { setReceiptAttachment(null); router.refresh(); } }); }} className="space-y-4">
            ...
            <Button className="w-full" aria-busy={pending} disabled={pending}>{pending ? "Guardando y archivando…" : "Adjuntar comprobante"}</Button>
          </form>
        </MobileDialog>
      )}
    </section>
  ...
  return (
    ...
    >
      <form
        ...
        className="space-y-4"
      >
        <Field label="Monto"><input max={maxAmount} min="1" name="amount" required type="number"/></Field><Field label="Fecha"><input defaultValue={chileInputDate()} name="paidOn" required type="date"/></Field><Field label="Método de pago"><select defaultValue="TRANSFER" name="method"><option value="TRANSFER">Transferencia</option><option value="CARD">Tarjeta</option><option value="CASH">Efectivo</option><option value="OTHER">Otro</option></select></Field><Field label="Comprobante"><input accept="image/jpeg,image/png,image/webp,application/pdf" name="receipt" type="file"/></Field><Field label="Observación (opcional)"><input name="observation"/></Field>{error&&<p aria-live="assertive" className="rounded-xl border border-danger/30 bg-danger/5 p-3 text-sm text-danger">{error}</p>}<Button className="w-full" aria-busy={pending} disabled={pending}>{pending ? "Registrando…" : "Guardar nuevo pago"}</Button>
      </form>
    </MobileDialog>
  );
}

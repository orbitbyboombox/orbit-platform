export interface BusinessIntelligenceDataset{
  generatedAt:string;
  financialEvents:readonly {project_id:string;customer_id:string;status:string;event_date:string|null;revenue:number;real_cost:number;personnel_cost:number;operational_resources_cost:number;total_operational_cost:number;gross_profit:number;gross_margin:number;outstanding_balance:number}[];

...
export async function loadIntegrationHealth(): Promise<IntegrationHealthSnapshot> {
  ...
  const [webhooks, states, communications, outbound, customers, connection] = await Promise.all([
  ...
  const whatsappConfigured = configured(["WHATSAPP_VERIFY_TOKEN", "WHATSAPP_APP_SECRET", "WHATSAPP_APP_ID", "WHATSAPP_GRAPH_VERSION", "WHATSAPP_PHONE_NUMBER_ID", "WHATSAPP_BUSINESS_ACCOUNT_ID", "WHATSAPP_ACCESS_TOKEN"]);
  const deliveryStatus = deliveryOn ? (connection.providerReady ? "PASS" : "ERROR") : "OFF";
  const deliveryDetail = deliveryOn ? (connection.providerReady ? "Delivery autorizado y proveedor verificado" : "Delivery bloqueado: proveedor/WABA no verificado") : "WHATSAPP_DELIVERY_ENABLED=false";
  ...
  return {
    ...
    summary: [
      ...
      signal("Delivery automático", deliveryStatus, deliveryDetail),
      ...
      signal("BIANCA Customer Messaging", biancaMessagingOn ? "PASS" : "OFF", biancaMessagingOn ? "Gate de mensajería activo" : "Fail-closed: mensajería autónoma desactivada"),
      signal("BIANCA Outbound", biancaOutboundOn ? "PASS" : "OFF", biancaOutboundOn ? "Gate de BIANCA outbound activo" : "Gate de salida desactivado"),
    ...
    whatsapp: [
      ...
      signal("WABA / Phone ID", connection.providerReady ? "PASS" : "PENDIENTE", connection.providerReady ? "Phone ID y suscripción verificados" : "WABA/Phone ID/token/suscripción pendientes"),
      ...
      signal("Delivery", deliveryStatus, deliveryDetail),
    ],
    boombox: [
    ...
    orbit: [
      ...
      signal("Health timestamp", "PASS", new Date().toLocaleString("es-CL")), signal("Delivery guard", deliveryStatus, deliveryDetail),
    ],
  };
}

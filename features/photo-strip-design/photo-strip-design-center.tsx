...
export function PhotoStripDesignCenter({ projectId, documents }: { projectId: string; documents: readonly PhotoStripDesignDocument[] }) {
  ...
  return <section className="rounded-2xl border border-brand/25 bg-brand/5 p-4 sm:p-5" data-photo-strip-design-center>
    ...
    {current ? <div className="mt-5 grid min-w-0 gap-4 rounded-2xl border bg-card p-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
      <div className="flex min-w-0 items-start gap-3"><FileImage className="mt-0.5 size-5 shrink-0 text-brand"/><dl className="grid min-w-0 gap-2 text-sm sm:grid-cols-[auto_minmax(0,1fr)] sm:items-baseline sm:gap-x-3"><dt className="text-muted">Archivo</dt><dd className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap font-semibold" title={current.originalFilename}>{current.originalFilename}</dd><dt className="text-muted">Versión</dt><dd className="whitespace-nowrap font-semibold">V{current.version} · CURRENT</dd><dt className="text-muted">Subido</dt><dd className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap">{dateTime(current.createdAt)} · {current.uploadedBy}</dd>{current.approvedAt?<><dt className="text-muted">Aprobado</dt><dd className="whitespace-nowrap">{dateTime(current.approvedAt)}</dd></>:null}<dt className="text-muted">Drive</dt><dd><StatusBadge label={current.driveStatus === "SYNCED" ? "ARCHIVADO" : current.driveStatus === "ERROR" ? "ERROR" : "PENDIENTE"} variant={current.driveStatus === "SYNCED" ? "success" : current.driveStatus === "ERROR" ? "danger" : "warning"}/></dd></dl></div>
      <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap lg:max-w-md lg:justify-end"><Button asChild variant="outline"><a href={current.href} rel="noreferrer" target="_blank"><Eye className="size-4"/>Ver diseño</a></Button><Button asChild variant="outline"><a href={`${current.href}?download=1`}><Download className="size-4"/>Descargar</a></Button>{status !== "APPROVED"?<Button aria-busy={pending} disabled={pending} onClick={approve}><CheckCircle2 className="size-4"/>Aprobar</Button>:null}<Button aria-busy={pending} disabled={pending} onClick={()=>setUploadOpen(true)} variant="outline"><Upload className="size-4"/>Nueva versión</Button></div>

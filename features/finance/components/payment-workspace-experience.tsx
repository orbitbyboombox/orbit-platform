...
export function PaymentWorkspaceExperience(props: PaymentWorkspaceExperienceProps) {
  ...
  return <WorkspaceLayout
    ...
    copilot={<OrbitCopilot actionLabel="Validar pago" estimatedTime="30 segundos" impact="La reserva podrá continuar solo después de la revisión BOOMBOX." onAction={() => setFeedback("Pago aprobado · demostración UI")} reason="El comprobante fue adjuntado por el cliente." recommendation="Validar pago" title="Acción BOOMBOX" />}
    ...
    timeline={null}
    bottomAction={<div className="grid gap-2 rounded-2xl border bg-card/95 p-4 shadow-xl backdrop-blur sm:grid-cols-3"><ActionButton icon={BadgeCheck} label="Validar Pago" onClick={() => setFeedback("Pago aprobado · demostración UI")} /><ActionButton icon={Ban} label="Rechazar Pago" onClick={() => setFeedback("Pago rechazado · demostración UI")} variant="outline" /><ActionButton icon={RefreshCw} label="Solicitar nuevo comprobante" onClick={() => setFeedback("Nuevo comprobante solicitado · demostración UI")} variant="outline" /></div>}
  />;
}

...
export function FinancialDashboardHeader({ data }: { data: FinanceDashboardReadModel }) {
  return <header className="rounded-3xl border bg-card p-6 sm:p-8" data-workspace-key="FINANCE_DASHBOARD" data-workspace-label="KPIs financieros" data-workspace-section>
    ...
    <div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[.14em]"><span className="rounded-full bg-brand/10 px-3 py-1.5 text-brand">Este mes · {data.periodLabel}</span><span className="rounded-full bg-background px-3 py-1.5 text-muted">Posición actual separada</span></div>
  </header>;
}

export function AvailableCashSection({ data }: { data: FinanceDashboardReadModel["cash"] }) {
  return <DashboardSection eyebrow="Detalle de posición" title="Caja registrada en ORBIT" description="Cobros menos egresos pagados registrados. No representa por sí sola el saldo bancario conciliado." workspaceKey="AVAILABLE_CASH">
...
export function PeriodMetricsSection({ eyebrow, title, description, metrics, workspaceKey }: { eyebrow: string; title: string; description: string; metrics: FinanceMetric[]; workspaceKey: string }) {
  return <DashboardSection eyebrow={eyebrow} title={title} description={description} workspaceKey={workspaceKey}><div className="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">{metrics.map((metric) => <MetricCard key={metric.label} metric={metric} />)}</div></DashboardSection>;
}
